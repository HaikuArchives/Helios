...
StandardConfigView::StandardConfigView(BRect frame) :
	ConfigView(frame)
{
	...
	// TEXTCONTROLS
	publisherTC=new BTextControl(BRect(15,8,frame.Width()-15,33),"scv_publisherTC",cdPublisherLabel.String(), "",NULL); // "TPUBLISHER"
	...
	preparerTC=new BTextControl(BRect(15,38,frame.Width()-15,63),"scv_preparerTC",cdPreparerLabel.String(), "",NULL); // "TPREPARER"
	...
	applicationTC=new BTextControl(BRect(15,68,frame.Width()-15,93),"scv_applicationTC",cdApplicationLabel.String(), "",NULL); // "TAPPLICATION"
...
//	projectpathTC=new BrowseField(BRect(15,98,340,123),"scv_projectpathTC");//,_T("TPROJECTPATH"), "",NULL);
...
//	temppathTC=new BrowseField(BRect(15,128,340,153),"scv_temppathTC");//,_T("TTEMPPATH"), "",NULL);
...
//	temppathTC->GetTextControl()->TextView()->SetMaxBytes(8192);
	...
	filesystemMF=new BMenuField(BRect(15,98,230,123),"scv_filesystemMF", cdFilesystemLabel.String(), filesystemPUM, false); // "TFILESYSTEM"
	...
	// VOLUMENAME
	volumenameTC=new BTextControl(BRect(15,128,frame.Width()-15,153),"scv_volumenameTC",cdVolumeNameLabel.String(), "",NULL); // "TVOLUMENAME"
...
void StandardConfigView::SetFilesystem(int32 item) {
	if (filesystemPUM->ItemAt(item)!=NULL)
		(filesystemPUM->ItemAt(item))->SetMarked(true);
}
