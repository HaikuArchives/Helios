...
DataCDPanel::DataCDPanel(BRect frame):BView(frame, "DataCDPanel", B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW) {
	multisessionCB=new BCheckBox(BRect(frame.left+4, frame.top, frame.right-4, frame.top+16), "dcdp_multisessionCB",
						_T("Multisession-CD"), // "TMULTISESSION"
...
BList	*DataCDPanel::GetFilesystem() {
	...
	switch (filesystemPUM->IndexOf(filesystemPUM->FindMarked())) {
		...
		// MacOS hfs
		case 3:	list->AddItem(new BString("-h")); break;
		// Unix RockRidge
...
void	DataCDPanel::SetMultisession(bool flag) {
	if (flag) 	multisessionCB->SetValue(B_CONTROL_ON); else
			...
			if (multisessionCB->Value()==B_CONTROL_ON) {
				...
				if (app->configW->Lock()) {
					if (app->heliosCV->IsDAO()) {
						...
						if (app->window1->Lock()) {
							app->window1->view1->statusBAR->SetText(_T("DAO disabled."), 4000000); // "L:DAO disabled."
							app->window1->Unlock();
						}
					}
					app->configW->Unlock();
