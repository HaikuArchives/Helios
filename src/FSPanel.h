...
public:
	...
	void SetJolietDefault(bool state)
	{
		JolietDef_RB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
	}
	void SetJolietUnicode(bool state)
	{
		JolietUni_RB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
	}

	void SetRockRidgeDefault(bool state)
	{
		RRDef_RB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
	}
	void SetRockRidgeRationalized(bool state)
	{
		RRRat_RB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
	}
...
private:
	BCheckBox* ISO9660_CB, *Joliet_CB, *RockRidge_CB, *UDF_CB, *HFS_CB, *Apple_CB, *BFS_CB,
		*mappingCB;
	BRadioButton* F83_RB, *F31_RB, *F37_RB, *JolietDef_RB, *JolietUni_RB, *RRDef_RB, *RRRat_RB,
		*UDFDef_RB, *UDFDVD_RB;
	BView* ISO9660_V, *Joliet_V, *RockRidge_V, *UDF_V, *HFS_V, *BFS_V;
