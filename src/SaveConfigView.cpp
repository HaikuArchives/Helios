...
SaveConfigView::SaveConfigView(BRect frame) : ConfigView(frame)
{
	// CHECKBOXES
	streamedCB = new BCheckBox(BRect(15, 5, 230, 20), "scv_streamedCB", _T("Data streaming"),
							   NULL); // "TSTREAMING"
	streamedCB->ResizeToPreferred();
	AddChild(streamedCB);
	savewhenquitCB =
		new BCheckBox(BRect(15, streamedCB->Frame().bottom + 5, 230, 40), "scv_savewhenquitCB",
					  _T("Remember last project"), NULL); // "TSAVEWHENQUIT"
	savewhenquitCB->ResizeToPreferred();
	AddChild(savewhenquitCB);
	autoupdateCB =
		new BCheckBox(BRect(15, savewhenquitCB->Frame().bottom + 5, 230, 60), "scv_autoupdateCB",
					  _T("Info box automatic refresh"), NULL); // "TAUTOUPDATEINFOBOX"
	...
	onexitdoSV = new BStringView(BRect(15, autoupdateCB->Frame().bottom + 5, 280, 80),
								 "scv_onexitdoSV", tempstring.String());
	onexitdoSV->ResizeToPreferred();
	AddChild(onexitdoSV);

	// RADIOBUTTONS
	autosaveRB =
		new BRadioButton(BRect(30, onexitdoSV->Frame().bottom + 5, 300, 100), "scv_autosaveRB",
						 _T("Save project without asking."), NULL); // "TAUTOSAVE_RB"
	autosaveRB->ResizeToPreferred();
	AddChild(autosaveRB);
	askforsaveRB = new BRadioButton(
		BRect(30, autosaveRB->Frame().bottom + 5, 300, 120), "scv_askforsaveRB",
		_T("Ask whether project should be saved or not."), NULL); // "TASKFORSAVE_RB"
	askforsaveRB->ResizeToPreferred();
	AddChild(askforsaveRB);
	donothingRB =
		new BRadioButton(BRect(30, askforsaveRB->Frame().bottom + 5, 300, 140), "scv_donothingRB",
						 _T("Nothing. Just quit."), NULL); // "TDONOTHING_RB"
	donothingRB->ResizeToPreferred();
	AddChild(donothingRB);

	// balloon help cb
	//	balloonhelpCB=new BCheckBox(BRect(15,150,230,165), "scv_balloonhelpCB", _T("Bubble help"),
	//// "CB:Bubble Help"
	//			new BMessage(BALLOONHELP_MSG), B_FOLLOW_LEFT|B_FOLLOW_TOP);
	//	AddChild(balloonhelpCB);

	burnPrioritySV = new BStringView(BRect(15, donothingRB->Frame().bottom + 15, frame.Width() - 15,
										   donothingRB->Frame().bottom + 30),
									 "scv_burningPrioritySV", _T("Burning Thread Priority"));
	AddChild(burnPrioritySV);

	burnPriority =
		new BSlider(BRect(15, burnPrioritySV->Frame().bottom + 5, frame.Width() - 15,
						  burnPrioritySV->Frame().bottom + 15),
					"scv_burnPrioritySL", NULL, new BMessage(BURNPRIORITYSLIDER_CHANGED), 1, 2);
...
bool SaveConfigView::IsStreamed()
{
	return streamedCB->Value() == B_CONTROL_ON;
}

void SaveConfigView::SetStreamed(bool state)
{
	streamedCB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
}

bool SaveConfigView::IsSavedWhenQuit()
{
	return savewhenquitCB->Value() == B_CONTROL_ON;
}

void SaveConfigView::SetSaveWhenQuit(bool state)
{
	savewhenquitCB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
}

int32 SaveConfigView::GetBurnPriority()
{
...
void SaveConfigView::SetBurnPriority(int32 priority)
{
	// Convert real priority to slider value.
	burnPriority->SetValue((priority == 21) ? 1 : 2);
}

exit_modes SaveConfigView::GetExitMode()
{
...
void SaveConfigView::SetExitMode(exit_modes mode)
{
	switch (mode) {
...
bool SaveConfigView::AutoUpdate()
{
	return autoupdateCB->Value() == B_CONTROL_ON;
}

void SaveConfigView::SetAutoUpdate(bool state)
{
	autoupdateCB->SetValue(state ? B_CONTROL_ON : B_CONTROL_OFF);
}

void SaveConfigView::SetBalloonHelp(bool state)
{
...
void SaveConfigView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
...
void SaveConfigView::AllAttached()
{
}
