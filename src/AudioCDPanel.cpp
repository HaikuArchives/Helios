...
AudioCDPanel::AudioCDPanel(BRect frame)
	: BView(frame, "AudioCDPanel", B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW)
{
	cdtextCB = new BCheckBox(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
							 "acdp_writeCDText", _T("Write CD text"), // "TWRITECDTEXT"
							 new BMessage(CDTEXTBOX_MSG));
	copyprotectionCB =
		new BCheckBox(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
					  "acdp_copyprotection", _T("Add SCMS-Copyright"), // "TCOPYPROTECTION"
					  NULL);
	CDTextAlbum =
		new BTextControl(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
						 "acdp_cdtextalbum", _T("CD text album"), // "TCDTEXTALBUMLABEL"
						 "", NULL);
