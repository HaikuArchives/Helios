...
FileChooser::FileChooser()
	...
	fileList=new BList();
	winView=new BView(Bounds(), "FileChooserView", B_FOLLOW_LEFT_RIGHT| B_FOLLOW_TOP_BOTTOM, B_WILL_DRAW);
	winView->SetViewColor(216,216,216);
	listView=new BColumnListView(BRect(0,42, 331, 251), "FileChooserCLV", B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP_BOTTOM, B_WILL_DRAW);
	okB=new BButton(BRect(253,7,323,34), "OK_Button", _T("Use"), new BMessage(OK_BUTTON_MSG), B_FOLLOW_TOP | B_FOLLOW_RIGHT);
	cancelB=new BButton(BRect(173,7,243,34), "CANCEL_Button", _T("Cancel"), new BMessage(B_QUIT_REQUESTED), B_FOLLOW_TOP | B_FOLLOW_RIGHT);
	browseB=new BButton(BRect(85,7,155,34), "SELECT_Button",_T("Browse") , new BMessage(BROWSE_BUTTON_MSG), B_FOLLOW_TOP | B_FOLLOW_RIGHT);
	...
	listView->SetColumnFlags((column_flags)(B_ALLOW_COLUMN_MOVE | B_ALLOW_COLUMN_RESIZE | B_ALLOW_COLUMN_POPUP | B_ALLOW_COLUMN_REMOVE));
