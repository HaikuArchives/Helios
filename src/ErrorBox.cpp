...
ErrorBox::ErrorBox(signal_color color, const char* title, const char* errortext,
				   const char* rightbutton, const char* middlebutton, const char* leftbutton)
	: BWindow((BRect(0, 0, 310, 170)), title, B_MODAL_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
			  B_NOT_ZOOMABLE | B_NOT_RESIZABLE)
{
	...
	textTV = new BTextView(BRect(45, 38, 308, textbottom), "textTV",
						   BRect(1, 1, 308 - 45, textbottom - 38), B_FOLLOW_LEFT | B_FOLLOW_TOP,
						   B_WILL_DRAW);
	...
	if (strlen(middlebutton) > 0) {
		middleB = new BButton(BRect(0, 0, 1, 1), "middleB", middlebutton,
							  new BMessage(MIDDLE_BUTTON_MSG));
		vError->AddChild(middleB);
		middleB->GetPreferredSize(&width, &height);
		middleB->ResizeTo(width, height);
		middleB->MoveTo((left = (left - width - 5)),
						textbottom + (180 - textbottom - height - 2) / 2);

		if (strlen(leftbutton) > 0) {
			leftB =
				new BButton(BRect(0, 0, 1, 1), "leftB", leftbutton, new BMessage(LEFT_BUTTON_MSG));
			vError->AddChild(leftB);
			leftB->GetPreferredSize(&width, &height);
			leftB->ResizeTo(width, height);
			leftB->MoveTo((left = (left - width - 5)),
						  textbottom + (180 - textbottom - height - 2) / 2);
		}
	}
...
void ErrorBox::MessageReceived(BMessage* msg)
{
	...
	case B_KEY_DOWN: {
		...
		if (msg->FindInt8("byte", &byte) == B_OK) {
			if (byte == lb)
				PostMessage(LEFT_BUTTON_MSG);
			else if (byte == mb)
				PostMessage(MIDDLE_BUTTON_MSG);
			else if (byte == rb)
				PostMessage(RIGHT_BUTTON_MSG);
		}
	}
