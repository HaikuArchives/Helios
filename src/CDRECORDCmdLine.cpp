...
status_t
CDRECORD_CommandLine(const char **argv, int *argc, bool multisession,	bool isStreaming)
{
	...
	// overburning		
	if (app->heliosCV->IsOverburning())
		argv[(*argc)++] = strdup("-overburn");
...
void
CDRECORD_ParseUserFlags(BString userFlags, BList &argList)
{
