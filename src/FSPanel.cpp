...
FSPanel::FSPanel(BRect frame) : ConfigView(frame)
{
	...
	// Controls
	ISO9660_CB =
		new BCheckBox(BRect(0, 0, 103, 20), "fsp_iso9660_cb", _T("ISO9660"), // "CB:ISO9660"
	...
	// View
	Joliet_V = new BView(BRect(oneThirdAcross, 5, 229, 100), "FSP_Joliet_V",
	...
	JolietDef_RB =
		new BRadioButton(BRect(15, 22, 112, 42), "fsp_JolietDef_RB", _T("Default"), // "RB:Default"
						 NULL);
	JolietDef_RB->SetValue(B_CONTROL_ON);
	Joliet_V->AddChild(JolietDef_RB);
	JolietUni_RB =
		new BRadioButton(BRect(15, 44, 112, 64), "fsp_JolietUni_RB", _T("Unicode"), // "RB:Unicode"
						 NULL);
	...
	// View
	RockRidge_V = new BView(BRect(twoThirdsAcross, 5, frame.Width() - 15, 100), "FSP_RockRidge_V",
							B_FOLLOW_LEFT | B_FOLLOW_TOP, 0);
	RockRidge_V->SetViewColor(ui_color(B_PANEL_BACKGROUND_COLOR));
	// Controls
	RockRidge_CB =
		new BCheckBox(BRect(0, 0, 112, 20), "fsp_RockRidge_CB", _T("RockRidge"), // "CB:RockRidge"
					  new BMessage(ROCKRIDGE_CB_MSG));
	RockRidge_V->AddChild(RockRidge_CB);
	RRDef_RB =
		new BRadioButton(BRect(15, 22, 112, 42), "fsp_RRDef_RB", _T("Default"), // "RB:Default"
						 NULL);
	RRDef_RB->SetValue(B_CONTROL_ON);
	RockRidge_V->AddChild(RRDef_RB);
	RRRat_RB = new BRadioButton(BRect(15, 44, 112, 64), "fsp_RRRat_RB",
								_T("Rationalized"), // "RB:Rationalized"
								NULL);
	...
	// View
	HFS_V = new BView(BRect(15, 105, frame.Width() - 15, 180), "FSP_HFS_V",
					  B_FOLLOW_LEFT | B_FOLLOW_TOP, 0);
	HFS_V->SetViewColor(ui_color(B_PANEL_BACKGROUND_COLOR));
	// Controls
	HFS_CB = new BCheckBox(BRect(0, 0, 100, 20), "fsp_HFS_CB",
						   _T("HFS/ISO Hybrid"), // "CB:HFS/ISO Hybrid"
						   new BMessage(HFS_CB_MSG));
	HFS_CB->ResizeToPreferred();
	HFS_V->AddChild(HFS_CB);
	mappingCB = new BCheckBox(BRect(15, 22, 170, 42), "fsp_mappingCB",
							  _T("Use mapping file:"), // "CB:Use Mapping File:"
							  new BMessage(MAPPING_CB_MSG));
	HFS_V->AddChild(mappingCB);

	BRect HFS_VRect = HFS_V->Bounds();
	mappingBF = new BrowseField(BRect(30, 44, HFS_VRect.Width(), 69), "fsp_mappingBF",
								B_FOLLOW_LEFT | B_FOLLOW_TOP, 0);
	...
	UDFDef_RB =
		new BRadioButton(BRect(15, 22, 120, 42), "fsp_UDFDef_RB", _T("Default"), // "RB:Default"
						 NULL);
	UDFDef_RB->SetValue(B_CONTROL_ON);
	UDF_V->AddChild(UDFDef_RB);
	UDFDVD_RB =
		new BRadioButton(BRect(15, 44, 120, 64), "fsp_UDFDVD_RB", _T("DVD Video"), // "RB:DVD Video"
						 NULL);
	...
	// View
	BFS_V = new BView(BRect(twoThirdsAcross, 190, frame.Width() - 15, 265), "FSP_BFS_V",
					  B_FOLLOW_LEFT | B_FOLLOW_TOP, 0);
