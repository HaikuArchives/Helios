...
BFileColumn::BFileColumn(const char* title, float width, float maxWidth, float minWidth,
						 uint32 truncate, alignment align, bool contextmenu)
...
int BFileColumn::CompareFields(BField* field1, BField* field2)
{
	...
	if (kFoldersFirst) {
		...
		if ((f1->IsFolder()) && (f2->IsFolder())) {
			if (f1->IsParentFolder())
				return -1;
			else if (f2->IsParentFolder())
				return 1;
			else
