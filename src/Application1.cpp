...
void Application1::AboutRequested(void)
{
	app_info info;
	AboutWindow* aboutW = new AboutWindow(BRect(100, 100, 420, 260), "About");
	BScreen* screen = new BScreen();

	aboutW->MoveTo((int)((screen->Frame().Width() - aboutW->Bounds().Width()) / 2),
				   (int)((screen->Frame().Height() - aboutW->Bounds().Height()) / 2));
	delete screen;
...
bool Application1::IsProject(const bool show_alert = true)
{
...
void Application1::SetHelpBubbles()
{
...
Application1::Application1() : BApplication("application/x-HeliosBurner.application"), CM(NULL)
{
...
//	HLP=new BubbleHelper();
	...
	if (string.Length() != 0)
		CM->SetString(PATH_TO_MKBFS, &string);
	else
		exit = true;
#else
	// mkbfs
	...
	if (!exit) {
		...
		//	window1TT->SetText(window1->view1->iconBAR->ChildViewAt("bar_update"), "Update
		//display");
...
void Application1::ReadyToRun()
{
	BString str;
	app_info ai;

	if (configW->Lock()) {
		versionCV->SetCDRecordVersion(
			VersionControl::GetToolVersion(CM->GetString(PATH_TO_CDRECORD)->String()),
			GetIcon(CM->GetString(PATH_TO_CDRECORD)->String()));
		versionCV->SetReadCDVersion(
			VersionControl::GetToolVersion(CM->GetString(PATH_TO_READCD)->String()),
			GetIcon(CM->GetString(PATH_TO_READCD)->String()));
		versionCV->SetMkISOfsVersion(
			VersionControl::GetToolVersion(CM->GetString(PATH_TO_MKISOFS)->String()),
			GetIcon(CM->GetString(PATH_TO_MKISOFS)->String()));
		versionCV->SetCDDA2WAVVersion(
...
bool Application1::QuitRequested(void)
{
...
void Application1::SetWorking(bool state)
{
	static int count = 0;
...
void Application1::MessageReceived(BMessage* message)
{
	if (applicationCV->IsAutoUpdating()) {
	...
	case FINISHED_EDITING_MSG: {
		...
		if (BString(selected->GetFilename()).Compare("..") != 0) {
			...
			if (file.SetTo(str.String(), false) == B_OK) {
				str = "";
				str << current_folder.String()
					<< ((BFileField*)selected->GetField(FILENAME_FIELD))->String();
	...
	case GOTOPARENT_MSG: {
		...
		if (current_folder.Length() <= base_folder.Length()) break;
		str = current_folder;
	...
	case 'mm03': // doppelklick auf listitem
	{
		...
		if ((window1->view1->CDType() == AUDIOCD_INDEX) ||
			(window1->view1->CDType() == CDEXTRA_INDEX)) {
	...
	case SAVEPROJECTAS_MIMSG: {
		...
		BEntry* entry =
			FileDialog::SaveDialog(NULL, pathCV->GetProjectPath(), SuggestProjectName());
		if (entry->InitCheck() == B_OK) {
			if (window1->Lock()) {
				BString tmpstring;
			...
			sprintf(cmdline, "cp -R -d \"%s\"* \"%s\"", original_base_folder.String(),
					base_folder.String());
			if (window1->Lock()) {
				BString tmpstring;
	...
	case 'mm14': // splits the selected track into two parts
	{
		...
		window1->Lock();
		window1->view1->listview1->AddRow(new AudioRow(ai->GetTrackNumber() + 1, ai->GetFilename(),
													   0, ai->GetBytes() - splitoffset,
													   (ai->GetBytes() - splitoffset) * 176400),
										  window1->view1->listview1->IndexOf(ai) + 1);
	...
	case HELIOS_LANGUAGE_UPDATE_MSG: {
		...
		for (int32 w = 0; w < CountWindows(); w++) {
			...
			for (int32 v = 0; v < window->CountChildren(); v++) {
				window->ChildAt(v)->Looper()->PostMessage(HELIOS_LANGUAGE_UPDATE_MSG,
														  window->ChildAt(v));
			}
			window->Unlock();
		}
...
void Application1::RefsReceived(BMessage* message)
{
	...
	if (message->FindRef("refs", 0, &ref) != B_OK) {
		if (window1->Alert(
				"Error at BMessage.FindRef()\n\nProbably you didn't drop a project file.") ==
			true) {
			delete entry;
			return;
		}
 ...
 *
 ***************************************************************************/
void Application1::ShowFolder(bool audio)
{
	BDirectory dir;
	BEntry de;
	BPath path;
	...
	dir.Rewind();
	window1->Lock();
	if ((audio) || ((window1->view1->CDType() == DATACD_INDEX) ||
	...
	while (dir.GetNextEntry(&de, false) == B_OK) {
		if (de.IsDirectory()) {
			...
			if ((window1->view1->CDType() == DATACD_INDEX) ||
				...
				de.GetRef(&ref);
				if (window1->Lock()) {
					window1->view1->listview1->AddRow(
						new FolderRow(path.Leaf(), 0, true, GetIcon(&ref)), (int32)0);
					window1->Unlock();
				}
				file.Unset();
			}
	...
	while (dir.GetNextEntry(&de, false) == B_OK) {
		if ((de.IsSymLink()) || (de.IsFile())) {
			...
			if ((window1->view1->CDType() == DATACD_INDEX) ||
				...
				if (window1->Lock()) {
					window1->view1->listview1->AddRow(
						new FolderRow(path.Leaf(), filesize, false, GetIcon(&ref)),
						window1->view1->listview1->CountRows());
					window1->Unlock();
				}
				file.Unset();
			} else if (audio) {
...
bool Application1::GetImageSize()
{
	int arguments = 0;
	thread_id Thread;
	const char** args;
	...
	case DATACD_INDEX: {
		...
		if (list->CountItems() == 1)
			if (((BString*)list->ItemAt(0))->Compare("BFS") == 0) {
				...
				delete ((BString*)list->RemoveItem((long int)0l));
				delete list;
				return true;
			}

		for (int i = list->CountItems() - 1; i >= 0; i--) delete ((BString*)list->RemoveItem(i));
		delete list;
...
int32 DataBuffer(void* data)
{
	...
	while ((d->working) && (bytesread1 > 0) && (written >= 0)) {
		...
		if (bytesread1 > 0) written = write(d->out, buffer1, bytesread1);
	}
...
status_t Application1::WaitForDisc(const char* device)
{
...
void Application1::ReadOutputOfCdrecord(int outstream, thread_id thread, int tracks)
{
	size_t bytesread = 0;
		...
		if (bytesread > 0) {
			...
			if (speed == 0)
				if (strstr(buffer, "at speed")) {
					...
					speed = atoi(s.String());
					sprintf(info, "%dx (%0.1f MB/s)", speed,
							153600.0 * (float)speed / (1024.0 * 1024.0));
					SetStatusInfo(info);
				}
...
status_t Application1::MakeImage()
{
	int arguments = 0;
	...
	arguments = 0;
	CDRECORD_CommandLine(args, &arguments, window1->view1->IsMultisession(), true /*isStreaming*/);
	...
	if (deviceCV->IsWriterEjected()) eject_media(deviceCV->GetWriterDevicePath());
	free(args);
	return B_OK;
}

status_t Application1::MakeImage_NOSTREAMING()
{
...
int Application1::ReadAudioCD()
{
				...
				if (percentage < oldpercentage) {
					...
					if (trackno > 1) {
						...
						if (size > 0) {
							ShowFolder(true);
							if (window1->Lock()) {
								// window1->view1->listview1->AddRow(new
								// AudioRow(window1->view1->listview1->iCountItems()+1,
								// leaf.String(), 0, size, size/176400));
								window1->Unlock();
							}
						}
						delete info;
	...
	if (deviceCV->IsReaderEjected()) eject_media(deviceCV->GetReaderDevicePath());
...
void Application1::ShowStatus(const char* text, thread_id Thread)
{
	if (fStatusWindow->Lock()) {
		...
		fStatusWindow->MoveTo(window1->Frame().left +
								  (window1->Bounds().Width() - fStatusWindow->Bounds().Width()) / 2,
							  window1->Frame().top +
								  (window1->Bounds().Height() - fStatusWindow->Bounds().Height()) /
									  2);
...
void Application1::SetStatus(const char* text, thread_id Thread)
{
	if (fStatusWindow->Lock()) {
		if (Thread != -1) {
			fStatusWindow->Thread = Thread;
...
void Application1::SetStatusInfo(const char* text, const char* leading)
{
	BString s = text;
	if (fStatusWindow->Lock()) {
...
void Application1::SetPercentage(float percentage)
{
	if (fStatusWindow->Lock()) {
		if (percentage > 100) percentage = 100;
		if (percentage < 0) percentage = 0;
		fStatusWindow->fStatusView->progressSB->Update(
			percentage - fStatusWindow->fStatusView->progressSB->CurrentValue());
		fStatusWindow->Unlock();
	}
}
// *** END OF STATUS WINDOW FUNCTIONS

void Application1::ChangePauses()
{
	float pause = 0;
...
void Application1::DoAudioMenu()
{
	window1->Lock();

	while (window1->view1->listview1->cmenu->CountItems() > 0)
		window1->view1->listview1->cmenu->RemoveItem((long int)0l);
...
void Application1::Update_DATA_AUDIO()
{
	if (window1->view1->listview1->cmenu == NULL) return;
	window1->Lock();
...
bool ReadData(BMediaFile* mfile, int fd, int64 start, int64 end)
{
	...
	if (track->EncodedFormat(&mf) == B_OK)
		printf("HELIOS: audio input is %0.0fHz/%ld channels\n", mf.u.raw_audio.frame_rate,
			   mf.u.raw_audio.channel_count);
	...
	while ((j < length) && (framesRead > 0)) {
		...
		if (framesRead > 0) write(fd, audioData, framesRead * 4);
		j += framesRead * 4;
		// printf("BUFFER: %lld/%lld\n", j, length);
	}
	for (int i = 0; i < buffersize; i++) audioData[i] = 0;
	while (j < length) {
		if (length - j > buffersize)
...
bool OpenFile(const char* filename, int fd, int64 start, int64 end)
{
	entry_ref ref;
	BMediaFile* mfile;
...
int32 AudioBufferProducer(void* data)
{
	...
	for (int i = 0; i < ap->trackcount; i++) {
		printf("HELIOS filename[%d/%d]: %s start: %lld stop: %lld\n", i, ap->trackcount,
			   ap->filename[i], ap->start[i], ap->end[i]);
		OpenFile(ap->filename[i], ap->fd, ap->start[i], ap->end[i]);
	}
	return 0;
}

status_t Application1::BurnAudio(bool multisession = false)
{
	int arguments = 0;
	...
	if (window1->view1->WriteCDText()) {
		...
		if (strlen(albumtitle) > 0) cdt->SetAlbum(albumtitle);
		for (int i = 0; i < window1->view1->listview1->iCountItems(); i++) {
			AudioRow* si = (AudioRow*)window1->view1->listview1->RowAt(i);
			if (strlen(si->GetCDTitle()) > 0) cdt->SetTrack(i + 1, si->GetCDTitle());
		}
	...
	for (int i = 0; i < window1->view1->listview1->iCountItems(); i++) {
		...
		if (!si->IsSplitted()) ap.start[arguments] = si->GetStartFrame();
	...
	buffer_Thread =
		spawn_thread(AudioBufferProducer, "Buffer Thread", BUFFER_PRIORITY, (void*)(&ap));
		...
		if (bytesread > 0) {
			...
			if (speed == 0)
				if (strstr(buffer, "at speed")) {
					...
					speed = atoi(s.String());
					sprintf(info, "%dx (%0.1f MB/s)", speed,
							176400.0 * (float)speed / (1024.0 * 1024.0));
					SetStatusInfo(info);
				}
...
void Application1::MoveUp()
{
						...
						(((AudioRow *)(window1->view1->listview1->RowAt(to)))->GetSplitIndex()==0))
	{
					...
					if (!((((AudioRow *)(window1->view1->listview1->RowAt(from)))->IsSplitted()) ||
						(((AudioRow
	*)(window1->view1->listview1->RowAt(from)))->GetSplitIndex()==0)))
			  ...
			  for (int32 i=window1->view1->listview1->CountItems()-2;i>=0;i--)
				if (window1->view1->listview1->IsItemSelected(i)) {
					...
					while ((((AudioItem *)(window1->view1->listview1->ItemAt(to)))->IsSplitted()) ||
						(((AudioItem
	*)(window1->view1->listview1->ItemAt(to)))->IsParentSplitterTrack())) {
					to++;
					};
					// move item when it is NOT a part of a splitted track
					if (!((((AudioItem *)(window1->view1->listview1->ItemAt(i)))->IsSplitted()) ||
						(((AudioItem
	*)(window1->view1->listview1->ItemAt(i)))->IsParentSplitterTrack())))
...
status_t Application1::ReadImage(BString* filename, bool alloweject)
{
			...
			if (strstr(buffer, "addr:")) {
				...
				if (length > 0) {
					if (progressed > olddone + 3072) {
						sprintf(info, "%0.1f MB/s", 2048.0 * (progressed - olddone) /
														(watch->ElapsedTime() * 1.048576));
						olddone = progressed;
						SetStatusInfo(info);
						watch->Reset();
					}
...
status_t Application1::MakeImageOnly(const char* filename)
{
	status_t error;
	BList* list;
	bool bfs = false;
	...
	case DATACD_INDEX: {
		...
		if (list->CountItems() == 1)
			if (((BString*)list->ItemAt(0))->Compare("BFS") == 0) bfs = true;
...
void Application1::eject_media(const char* dev)
{
	int fd;
	status_t status;
...
void Application1::load_media(const char* dev)
{
	int fd;
	status_t status;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
status_t Application1::CopyAudioCD()
{
...
status_t Application1::CopyDataCD()
{
...
status_t Application1::CopyOnTheFlyAudio()
{
	int arguments = 0;
	...
	// Make CDRECORD options
	CDRECORD_CommandLine(args, &arguments, window1->view1->IsMultisession(), false /*isStreaming*/);
...
status_t Application1::CopyOnTheFly()
{
	// wait till a disc is in the drive and drive is ready
	if (WaitForDisc(deviceCV->GetWriterDevicePath()) == B_TIMEOUT) {
		errtype = B_TRANSLATE("Timeout");												 // "TTIMEOUTTYPE"
...
size_t FreeVolumeSpace(const char* path)
{
	fs_info info;
	dev_t device;
...
status_t Application1::MakeMKISOImage(const char* filename)
{
	const char** args;
	int arguments = 0;
	thread_id Thread;
...
void Application1::SaveSettings()
{
...
void Application1::ReadSettings()
{
	BMessage* msg;
...
*/
	...
	if (!CM->HasData(BURNPRIORITY)) {
		CM->SetInt32(BURNPRIORITY, 21);
	}

	// ColorConfigView
	if (!CM->HasData(BURNING_STATUS_BAR_COLOR))
		CM->SetColor(BURNING_STATUS_BAR_COLOR, (rgb_color){60, 225, 60, 255});
	if (!CM->HasData(CDSIZE_STATUS_BAR_COLOR))
		CM->SetColor(CDSIZE_STATUS_BAR_COLOR, (rgb_color){50, 150, 255, 255});
	if (!CM->HasData(LOGWIN_BG_COLOR))
		CM->SetColor(LOGWIN_BG_COLOR, (rgb_color){255, 255, 255, 255});
	if (!CM->HasData(LOGWIN_FG_COLOR)) CM->SetColor(LOGWIN_FG_COLOR, (rgb_color){0, 0, 0, 255});
	if (!CM->HasData(LISTSELECTION_COLOR))
		CM->SetColor(LISTSELECTION_COLOR, (rgb_color){90, 90, 90, 255});
	if (!CM->HasData(LISTBACKGROUND_COLOR))
		CM->SetColor(LISTBACKGROUND_COLOR, (rgb_color){255, 255, 255, 255});

	// CD-MODE-SPECIFIC CONFIGURATION
	// DATA-CD
	if (!CM->HasData(DATACD_FILESYSTEM))
		CM->SetInt32(DATACD_FILESYSTEM, CM->GetInt32(S_FILESYSTEM));
	if (!CM->HasData(DATACD_VOLUMENAME))
		CM->SetString(DATACD_VOLUMENAME, CM->GetString(S_VOLUMENAME)->String());
	...
	// BOOTABLE-CD
	if (!CM->HasData(BOOTCD_FILESYSTEM))
		CM->SetInt32(BOOTCD_FILESYSTEM, CM->GetInt32(S_FILESYSTEM));
	if (!CM->HasData(BOOTCD_VOLUMENAME))
		CM->SetString(BOOTCD_VOLUMENAME, CM->GetString(S_VOLUMENAME)->String());
	...
	if (!CM->HasData(CDEXTRA_FILESYSTEM))
		CM->SetInt32(CDEXTRA_FILESYSTEM, CM->GetInt32(S_FILESYSTEM));
	if (!CM->HasData(CDEXTRA_VOLUMENAME))
		CM->SetString(CDEXTRA_VOLUMENAME, CM->GetString(S_VOLUMENAME)->String());
	// DATA-DVD
	if (!CM->HasData(DVDVIDEO_FILESYSTEM))
		CM->SetInt32(DVDVIDEO_FILESYSTEM, CM->GetInt32(S_FILESYSTEM));
	if (!CM->HasData(DVDVIDEO_VOLUMENAME))
		CM->SetString(DVDVIDEO_VOLUMENAME, CM->GetString(S_VOLUMENAME)->String());
	...
	if (CM->HasData(MAINWINDOW_RECT)) {
		window1->ResizeTo(CM->GetRect(MAINWINDOW_RECT)->Width(),
						  CM->GetRect(MAINWINDOW_RECT)->Height());
	...
	if (CM->HasData(PREFERENCESWINDOW_RECT)) {
		 ...
		 * with wider prefs.
		 */
		// configW->ResizeTo(CM->GetRect(PREFERENCESWINDOW_RECT)->Width(),
		// CM->GetRect(PREFERENCESWINDOW_RECT)->Height());
		configW->MoveTo(CM->GetRect(PREFERENCESWINDOW_RECT)->left,
						CM->GetRect(PREFERENCESWINDOW_RECT)->top);
	}
	if (CM->HasData(CDRECORDLOGWINDOW_RECT)) {
		cdrecord_output->ResizeTo(CM->GetRect(CDRECORDLOGWINDOW_RECT)->Width(),
								  CM->GetRect(CDRECORDLOGWINDOW_RECT)->Height());
		cdrecord_output->MoveTo(CM->GetRect(CDRECORDLOGWINDOW_RECT)->left,
								CM->GetRect(CDRECORDLOGWINDOW_RECT)->top);
	}
	if (CM->HasData(MKISOFSLOGWINDOW_RECT)) {
		mkisofs_output->ResizeTo(CM->GetRect(MKISOFSLOGWINDOW_RECT)->Width(),
								 CM->GetRect(MKISOFSLOGWINDOW_RECT)->Height());
		mkisofs_output->MoveTo(CM->GetRect(MKISOFSLOGWINDOW_RECT)->left,
							   CM->GetRect(MKISOFSLOGWINDOW_RECT)->top);
	}
...
void Application1::Calculate_time()
{
...
void Application1::UpdateTrackNumbers()
{
...
int Application1::DeviceStatus(const char* path)
{
...
void Application1::AddToRecent(const char* filename)
{
	int i;
	...
	CM->SetStringI(RECENTPROJECTS_LIST, new BString(filename),
				   CM->CountStrings(RECENTPROJECTS_LIST));
}

void Application1::ShowRecentProjects()
{
	char* filename;
	filename = (char*)malloc(B_FILE_NAME_LENGTH);

	window1->Lock();
	while (window1->view1->openrecentSM->CountItems() > 0)
		delete window1->view1->openrecentSM->RemoveItem((long int)0l);
...
const char* Application1::SuggestProjectName()
{
	switch (window1->view1->CDType()) {
	case DATACD_INDEX: {
...
void Application1::LoadProject(BEntry* entry)
{
	if (!entry->Exists()) {
		return;
	}
	...
	if (projM->HasData("base_folder")) base_folder = projM->GetString("base_folder")->String();
	if (projM->HasData("publisher")) imageCV->SetPublisher(projM->GetString("publisher")->String());
	if (projM->HasData("preparer")) imageCV->SetPreparer(projM->GetString("preparer")->String());
	...
	case AUDIOCD_INDEX: {
		if (projM->HasData("writecdtext")) window1->view1->SetCDText(projM->GetBool("writecdtext"));
		...
		for (int32 t = 0; t < projM->CountStrings("filename"); t++) {
			AudioRow* item = new AudioRow(
				t + 1, projM->GetStringI("filename", t)->String(), *(projM->GetInt64("pause", t)),
				(off_t)(*(projM->GetInt64("size", t))), (off_t)(*(projM->GetInt64("length", t))));
	...
	if (saveCV->AutoUpdate()) window1->view1->box2->Refresh();
...
void Application1::SaveProject(BEntry* entry, bool export_project)
{
	...
	if (!export_project) current_folder.Append(subdir);
...
void Application1::SetAsStandardProject()
{
	/*	standardCV->
...
status_t Application1::GetMultisessionSectors(int32& start, int32& end)
{
	int arguments = 0;
	thread_id Thread;
	const char** args;
...
void Application1::ExportAsM3U(BEntry* entry)
{
	BFile* file = new BFile(entry, B_READ_WRITE | B_ERASE_FILE | B_CREATE_FILE);
...
void Application1::ExportAsAPML(BEntry* entry)
{
	BFile* file = new BFile(entry, B_READ_WRITE | B_ERASE_FILE | B_CREATE_FILE);
	M3UList* list = new M3UList();
	BString path = "";
...
void Application1::GetEntryNamesIn(BDirectory dir, M3UList* list)
{
...
void Application1::ExportAsFileList(BEntry* entry)
{
	BFile* file = new BFile(entry, B_READ_WRITE | B_ERASE_FILE | B_CREATE_FILE);
	M3UList* list = new M3UList();
	BEntry* musicfile = new BEntry();
...
bool Application1::CheckCDSize()
{
	int64 maxsize = 80;
	int blocksize;
	int64 maxsize_bytes;

	if (heliosCV->IsOverburning()) maxsize = 90;
	switch (window1->view1->CDType()) {
	case DATACD_INDEX: {
		blocksize = 2048;
		break;
...
void Application1::RemoveInfFiles()
{
	BEntry* entry = new BEntry();
...
BString SubString(BString* string, int32 first, int32 last)
{
	BString temp;
...
void Application1::PopulateFoldersMenu()
{
	...
	while (current.Length() > base_folder.Length()) {
		...
		mi = new BitmapMenuItem(
			SubString(&current, current.FindLast("/") + 1, current.Length()).String(),
			GetIcon(&ref), msg);
...
size_t Application1::GetDeviceSize(const char* path)
{
...
status_t Application1::BurnCDExtra()
{
...
void Application1::DeleteAndCount(BDirectory dir)
{
	const int32 c = dir.CountEntries();
	if (c > 0) {
		for (int32 i = 0; i < c; i++) {
...
void Application1::PlayFinishedBurningSound()
{
...
void Application1::PlayFinishedImageSound()
{
...
void Application1::PlayErrorSound()
{
...
status_t Application1::ReadFloppyImage(const char* path)
{
	BFile* src, *dst;
	ssize_t numread;
	...
	if ((src->InitCheck() == B_OK) && (dst->InitCheck() == B_OK)) {
		...
		while ((dst->Write(buffer, numread) == numread) && (buffersize >= sectorsize)) {
			...
			if (progressed > 18 * 512) {
				sprintf(info, "%0.1f kB/s",
						((float)progressed) / (((float)watch->ElapsedTime()) * 0.001024F));
				SetStatusInfo(info);
				watch->Reset();
				sum += progressed;
