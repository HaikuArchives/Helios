...
EditView::EditView(BRect frame, const char* name, const char* text, BMessage* invokeMsg,
				   uint32 resizingMode)
	: BView(frame, name, resizingMode, B_WILL_DRAW)
{

	if (text) fText = text;
	if (invokeMsg) fInvocationMsg = invokeMsg;
...
{
	if (fInvocationMsg) delete fInvocationMsg;
	fInvocationMsg = msg;
}
...
TextEditView::TextEditView(EditView* parent)
	: BTextView(parent->Bounds(), "TextEditView", parent->Bounds(), B_FOLLOW_LEFT | B_FOLLOW_TOP,
