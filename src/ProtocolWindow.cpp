...
ProtocolView::ProtocolView(BRect frame, const char *name, BRect textRect, uint32 resizingMode, uint32 flags, int stream)
...
bool ProtocolWindow::QuitRequested() {
	if (IsHidden()) { MoveTo(100000,100000);Show();Hide();}
	while (!IsHidden()) Hide();
	SetPulseRate(0);
...
/*	int32 index=0;
	while ((index=(s->FindFirst("\r", index)))!=B_ERROR) {
		...
		s->Remove(s->FindLast("\n",index)+1, ((s->FindFirst("\r", index)+1))-(s->FindLast("\n",index)+1));
