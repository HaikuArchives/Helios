...
uint16 updcrc(uint p_crc, register uint8* cp, register uint cnt)
{
	register uint16 crc = p_crc;
...
/*LOCAL UInt16_t*/
uint16 calcCRC(uchar* buf, uint bsize)
{
	return updcrc(0x0000, (uint8*)buf, bsize);
}
...
{
	uint16 crc = calcCRC(buf, bsize - 2);
