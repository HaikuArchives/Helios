...
AboutWindow::AboutWindow(BRect frame, const char* title)
	: BWindow(frame, title, B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
			  B_NOT_ZOOMABLE | B_NOT_RESIZABLE)
{
	...
	applicationNameSV =
		new BStringView(BRect(60, 1, (frame.Width() - 60), 27), "applicationNameSV", "");
	applicationNameSV->SetFont(be_bold_font);
	vAbout->AddChild(applicationNameSV);

	versionNumberSV =
		new BStringView(BRect(applicationNameSV->StringWidth(applicationNameSV->Text()) + 65, 1,
							  frame.Width() - 5, 27),
						"versionNumberSV", "");
	...
	copyrightStringSV =
		new BStringView(BRect(60, 27, frame.Width() - 20, 41), "copyrightStringSV", "");
	copyrightStringSV->SetFontSize(10);
	vAbout->AddChild(copyrightStringSV);

	textTV = new BTextView(
		BRect(60, 55, frame.Width() - 8 - B_V_SCROLL_BAR_WIDTH, frame.Height() - 7), "textTV",
...
void AboutWindow::SetApplicationName(const char* name)
{
	...
	versionNumberSV->ResizeTo((Bounds().Width() - 5) -
								  (applicationNameSV->StringWidth(applicationNameSV->Text()) + 65),
							  26);
	versionNumberSV->MoveTo(applicationNameSV->StringWidth(applicationNameSV->Text()) + 65,
							applicationNameSV->Frame().top -
								(boldheight.descent - plainheight.descent));
}
