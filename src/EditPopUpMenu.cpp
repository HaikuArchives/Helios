...
EditPopUpMenu::EditPopUpMenu(const char* name, bool radioMode, bool labelFromMarked,
							 menu_layout layout)
...
{
	if (fRemoveMsg) delete fRemoveMsg;
}
		...
		for (int32 i = CountItems() - 1; i >= 0; i--) {
			...
			if (is_instance_of(item, EditMenuItem)) {
				...
				if (item->Frame().Contains(location)) {
					...
					if (msg != NULL) fRemoveMsgTarget->Looper()->PostMessage(msg, fRemoveMsgTarget);
					break;
				}
			}
		}
