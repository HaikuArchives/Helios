...
SoundConfigView::SoundConfigView(BRect frame) : ConfigView(frame)
{
	// CHECKBOXES
	writtensoundCB =
		new BCheckBox(BRect(15, 5, 230, 20), "scv_writtensoundCB",
					  _T("Beep when finished recording disc"), // "TBEEPWHENFINISHEDDISC"
					  NULL);
	writtensoundCB->ResizeToPreferred();
	AddChild(writtensoundCB);
	imagesoundCB =
		new BCheckBox(BRect(15, writtensoundCB->Frame().bottom + 5, 230, 40), "scv_imagesoundCB",
					  _T("Beep when finished creating iImage"), // "TBEEPWHENFINISHEDIMAGE"
					  NULL);
	imagesoundCB->ResizeToPreferred();
	AddChild(imagesoundCB);
	errorsoundCB =
		new BCheckBox(BRect(15, imagesoundCB->Frame().bottom + 5, 230, 60), "scv_errorsoundCB",
