...
//HUGH
enum LatchStyle {
	B_TRIANGLE_LATCH,		// right/down-triangle (closed/open) as used in most BeOS-apps, BeIDE, and on Macs
...
typedef enum {
	...
	B_ALLOW_COLUMN_EDIT   = 16		//HUGH uses dynamic View (provided by column) for editing - see OutlineView::MouseDown()
} column_flags;
...
public:
	...
	// if edit == false-> user finished editing of field -> read changed data from field and store it
...
//			void		SetColumnName(const char *name);	//?HUGH should also be there?
	...
	/*!
		\param field The BField derivative to validate.
			...
			determine if it is of a kind that your BColumn know how to handle. return false if it is not.
...
public:
	...
	// Appearance (NEW STYLE)
			...
			void			SetColorBands(int bands);					//HUGH set number of bands used for color-banding of rows (max 8)
	...
	// Editing
			...
			*/
			...
			void			EndEditing();
	// Field manipulation
			// finds field at position -> returns field (+column) and updates Rect in place, NULL if not found
			// optionally also returns Row with indent and top-offset
			BField*			FindField(BPoint position, BRect* rect,
									  BRow** outRow = NULL,
									  int32* rowIndent = NULL, float* rowTop = NULL);
