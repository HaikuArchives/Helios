...
int32 UpdaterThread(void* data)
{
	...
	while (true) {
		...
		if (last_sec != (int)((currtime = audioW->audioAO->Track()->CurrentTime()) / 1000000)) {
			str = "";
			str << currtime / 60000000 << ":" << (((currtime / 1000000) % 60 < 10) ? "0" : "")
				<< (currtime / 1000000) % 60;
		...
		if (last_rsec !=
			(int)((duration - (currtime = audioW->audioAO->Track()->CurrentTime())) / 1000000)) {
			str = "";
			str << "-" << (duration - currtime) / 60000000 << ":"
				<< ((((duration - currtime) / 1000000) % 60 < 10) ? "0" : "")
				<< ((duration - currtime) / 1000000) % 60;
		...
		if ((audioW->status == 1) &&
			(audioW->audioAO->Track()->CurrentFrame() >= audioW->audioAO->Track()->CountFrames()))
...
AudioWindow::AudioWindow(const char* audiofile, AudioRow* ar)
	: BWindow(BRect(0, 0, 490, 300), "", B_FLOATING_WINDOW_LOOK, B_MODAL_APP_WINDOW_FEEL,
			  B_NOT_ZOOMABLE | B_NOT_RESIZABLE | B_ASYNCHRONOUS_CONTROLS)
{
	...
	volumeS = new BSlider(BRect(183, 18, 220, 89), "volumeS", NULL,
						  new BMessage(VOLUME_CHANGED_MSG), 0, 200);
	...
	rewindTB = new TransportButton(
		BRect(x1, LOWERLINE, x2, LOWERLINE + kRewindBitmapHeight - 1), "aw_rewindTB",
		kSkipBackBitmapBits, kPressedSkipBackBitmapBits, kDisabledSkipBackBitmapBits,
		new BMessage(REWINDED_MSG), new BMessage(STARTREWIND_MSG), new BMessage(REWINDING_MSG),
		new BMessage(NOTREWINDING_MSG), MSG_PERIOD, 0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	...
	playPPB = new PlayPauseButton(BRect(x1, LOWERLINE, x2, LOWERLINE + kPlayPauseBitmapHeight - 1),
								  "aw_playPPB", kPlayButtonBitmapBits, kPressedPlayButtonBitmapBits,
								  ...
								  kPressedPausedPlayButtonBitmapBits, new BMessage(PLAYPAUSE_MSG),
								  0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	...
	stopTB = new TransportButton(BRect(x1, LOWERLINE, x2, LOWERLINE + kStopBitmapHeight - 1),
								 "aw_stopTB", kStopButtonBitmapBits, kPressedStopButtonBitmapBits,
								 kDisabledStopButtonBitmapBits, new BMessage(STOP_MSG), NULL, NULL,
								 NULL, 0, 0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	playerBOX->AddChild(stopTB);

	// forward button
	x1 = x2 + 8;
	x2 = x1 + kRewindBitmapWidth - 1;
	forwardTB = new TransportButton(
		BRect(x1, LOWERLINE, x2, LOWERLINE + kRewindBitmapHeight - 1), "aw_forwardTB",
		kSkipForwardBitmapBits, kPressedSkipForwardBitmapBits, kDisabledSkipForwardBitmapBits,
		new BMessage(FORWARDED_MSG), new BMessage(STARTFORWARD_MSG), new BMessage(FORWARDING_MSG),
		new BMessage(NOTFORWARDING_MSG), MSG_PERIOD, 0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	...
	timeSV = new BStringView(
		BRect(1 * l + 8 + 1, LOWERLINE - 24, playerBOX->Bounds().Width() - 38, LOWERLINE - 7),
		"aw_timeSV", "0", B_FOLLOW_LEFT | B_FOLLOW_TOP);
	...
	remainingSV = new BStringView(BRect(8, LOWERLINE - 24, l + 8 - 1, LOWERLINE - 7),
								  "aw_remainingSV", "0", B_FOLLOW_LEFT | B_FOLLOW_TOP);
	...
	prevframeTB = new TransportButton(BRect(5, LOWERLINE - 30 - kRewindBitmapHeight,
											5 + kRewindBitmapWidth - 1, LOWERLINE - 30 - 1),
									  "aw_prevframeTB", kSkipBackBitmapBits,
									  kPressedSkipBackBitmapBits, kDisabledSkipBackBitmapBits,
									  new BMessage(REWINDEDF_MSG), new BMessage(STARTREWINDF_MSG),
									  new BMessage(REWINDINGF_MSG), new BMessage(NOTREWINDINGF_MSG),
									  MSG_PERIOD * 2, 0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	playerBOX->AddChild(prevframeTB);

	nextframeTB = new TransportButton(
		BRect(playerBOX->Bounds().Width() - 35 - kRewindBitmapWidth + 1,
			  LOWERLINE - 30 - kRewindBitmapHeight, playerBOX->Bounds().Width() - 35,
			  LOWERLINE - 30 - 1),
		"aw_nextframeTB", kSkipForwardBitmapBits, kPressedSkipForwardBitmapBits,
		kDisabledSkipForwardBitmapBits, new BMessage(FORWARDEDF_MSG),
		new BMessage(STARTFORWARDF_MSG), new BMessage(FORWARDINGF_MSG),
		new BMessage(NOTFORWARDINGF_MSG), MSG_PERIOD * 2, 0, 0, B_FOLLOW_LEFT | B_FOLLOW_TOP);
	playerBOX->AddChild(nextframeTB);

	frameTC = new BTextControl(
		BRect(5 + kRewindBitmapWidth - 3 + 5, LOWERLINE - 30 - kRewindBitmapHeight + 1,
			  playerBOX->Bounds().Width() - 35 - kRewindBitmapWidth - 5 - 1, LOWERLINE - 30 - 4),
		"aw_frameTC", NULL, "0", new BMessage(INDEX_ACCEPT_MSG));
	...
	for (int i = 0; i < 255; i++) frameTC->TextView()->DisallowChar(i);
	...
	addB = new BButton(BRect(5, 5, 5, 5), "aw_addB", _T("Add"), new BMessage(ADD_MSG),
					   B_FOLLOW_LEFT | B_FOLLOW_TOP);
	indexBOX->AddChild(addB);
	addB->ResizeToPreferred();
	addB->MoveTo(indexLV->Frame().right + B_V_SCROLL_BAR_WIDTH + 8 + 5, 6 + 11);
	removeB = new BButton(BRect(5 + addB->Frame().right, 5, 5, 5), "aw_removeB", _T("Delete"),
						  new BMessage(REMOVE_MSG), B_FOLLOW_LEFT | B_FOLLOW_TOP);
	indexBOX->AddChild(removeB);
	removeB->ResizeToPreferred();
	removeB->MoveTo(indexLV->Frame().right + B_V_SCROLL_BAR_WIDTH + 8 + 5, 38 + 11);

	// pauseBOX
	pauseTC = new BTextControl(
		BRect(5, 15, pauseBOX->Bounds().right - 7, pauseBOX->Bounds().bottom - 5), "pauseTC",
	...
	// cdtextBOX
	cdtextTC = new BTextControl(
		BRect(5, 15, cdtextBOX->Bounds().right - 7, cdtextBOX->Bounds().bottom - 5), "cdtextTC",
	...
	startTC =
		new BTextControl(BRect(8, 6 + 14, 78, 6 + 14), "startTC", _T("Start:"), // "B:Start"
	...
	selectionstartB->ResizeTo(selectionstartB->StringWidth(selectionstartB->Label()) + 28,
							  selectionstartB->Bounds().Height());
	...
	selectionendB->ResizeTo(selectionendB->StringWidth(selectionendB->Label()) + 28,
							selectionendB->Bounds().Height());
	selectionendB->MoveTo(83, 38 + 11);

	//	BChannelSlider	*oc=new BChannelSlider(BRect(x2+5,5,Bounds().right-5,Bounds().Height()-5),
	//"name", "Volume", NULL, B_VERTICAL, 2);
	//	mainV->AddChild(oc);
	indexL = new BList();
...
AudioWindow::~AudioWindow()
{
	...
	if (audioAO->IsPlaying()) audioAO->Stop();
	delete audioAO;
...
void AudioWindow::PlayChunk()
{
	...
	if (status == 2) {
		audioAO->Play();
		while (audioAO->Track()->CurrentTime() < time + 70000) snooze(1000);
		audioAO->Stop();
...
void AudioWindow::MessageReceived(BMessage* msg)
{
	...
	case INDEX_ACCEPT_MSG: {
		...
		if (str.FindFirst("s") != B_ERROR) {
			...
			audioAO->SeekToTime(&time);
		} else if (str.FindFirst("f") != B_ERROR) {
			str.RemoveFirst("f");
	...
	case SELECTIONSTART_MSG: {
		...
		startTC->SetText(pos);
		fAudioRow->SetStartFrame(
			(int64)(75.0F * (float)audioAO->Track()->CurrentTime() / 1000000.0F));
		project_has_changed = true;
		break;
	}
