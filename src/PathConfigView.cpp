...
PathConfigView::PathConfigView(BRect frame) : ConfigView(frame){
	// TEXTCONTROLS
	projectpathTC=new BrowseField(BRect(15,5,frame.Width()-15,30),"pcv_projectpathTC");//,_T("TPROJECTPATH"), "", new BMessage(PROJECTPATH_CHANGED));
	projectpathTC->GetTextControl()->SetDivider(projectpathTC->StringWidth(_T("Project path:"))+5); // "TPROJECTPATH"
	...
	temppathTC=new BrowseField(BRect(15,35,frame.Width()-15,60),"pcv_temppathTC");//,_T("TTEMPPATH"), "", new BMessage(TEMPPATH_CHANGED));
	temppathTC->GetTextControl()->SetDivider(temppathTC->StringWidth(_T("Temp. Path:"))+5); // "TTEMPPATH"
	...
	imagenameTC=new BrowseField(BRect(15,65,frame.Width()-15,90),"pcv_imagenameTC");//,_T("TDEFAULTIMAGENAME"), "",NULL);
	imagenameTC->GetTextControl()->SetDivider(imagenameTC->StringWidth(_T("Default image file name:"))+5); // "TDEFAULTIMAGENAME"
	...
	mountpointTC=new BrowseField(BRect(15,95,frame.Width()-15,120),"pcv_mountpointTC");//,_T("TDEFAULTMOUNTPOINT"), "",NULL);
	mountpointTC->GetTextControl()->SetDivider(mountpointTC->StringWidth(_T("Default mount point:"))+5); // TDEFAULTMOUNTPOINT"
