...
IconButton::IconButton(BRect frame, const char* name, BBitmap* icon, BMessage* message,
...
int32 IconBar::AddButton(const char* iconname, const char* name, BMessage* msg, uint32 resizingMode,
						 bool miniicon)
{
	BRect rect, btnrect;
	const void* ptr = NULL;
	...
	if (ptr) bm->SetBits(ptr, size, 0, B_CMAP8);
	this->AddChildView(new IconButton(btnrect, name, bm, msg, resizingMode));
	return fButtonL->CountItems() - 1;
}

int32 IconBar::AddButton(int32 ID, const char* name, BMessage* msg, uint32 resizingMode,
						 bool miniicon)
{
	BRect rect, btnrect;
	const void* ptr = NULL;
...
int32 IconBar::AddButtonMSGG(const char* iconname, const char* name, BMessage* msg,
							 uint32 resizingMode)
{
	BRect btnrect;
	const void* ptr = NULL;
