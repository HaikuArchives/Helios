...
ListView1::ListView1()
	: BColumnListView(BRect(0, 0, 302, 213 - B_H_SCROLL_BAR_HEIGHT), "listview", B_FOLLOW_ALL_SIDES,
					  B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE, B_NO_BORDER, true)
{
	...
	CDTEXTTitleItem =
		new BMenuItem(_T("CD text title"), new BMessage(SETCDTEXTTITLE_MSG)); // "TCDTEXTTITLE"
...
void ListView1::AllAttached()
{
	//	SetEditMode(false);
	MakeFocus(false);
	SetColumnFlags((column_flags)(B_ALLOW_COLUMN_MOVE | B_ALLOW_COLUMN_RESIZE |
...
void ListView1::SelectionChanged(void)
{

	Window1* window1 = (Window1*)Window();

	((BButton*)window1->view1->iconBAR->ChildViewAt("bar_deleteitem"))
		->SetEnabled(CurrentSelection() != NULL);
	// be_app->PostMessage('mm05');
}

void ListView1::KeyDown(const char* bytes, int32 numByte)
{

	if (bytes[0] == B_DELETE) be_app->PostMessage(DELETE_FILE_MSG);
		...
		/*
				switch(col->LogicalFieldNum()) {
					case TRACKNUMBER_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(TRACKNUMBERCOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					case TRACKNAME_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(TRACKNAMECOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					case LENGTH_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(LENGTHCOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					case BYTES_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(BYTESCOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					case INDEX_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(INDEXCOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					case CDTEXT_FIELD: {
						((Application1 *)be_app)->CM->SetInt16(CDTEXTCOLUMN_WIDTH,
		   (int16)col->Width());
						break;
					}
					default: {
...
void ListView1::SetListMode(int32 mode)
{
	...
	*/
	...
	((BButton*)window1->view1->iconBAR->ChildViewAt("bar_parent"))
		->SetEnabled((mode == DATACD_INDEX) || (mode == BOOTABLECD_INDEX) ||
					 (mode == DVDVIDEO_INDEX));
	((BButton*)window1->view1->iconBAR->ChildViewAt("bar_newfolder"))
		->SetEnabled((mode == DATACD_INDEX) || (mode == BOOTABLECD_INDEX) ||
					 (mode == DVDVIDEO_INDEX));
	((BTextControl*)window1->view1->iconBAR->ChildViewAt("bar_path"))
		->SetEnabled((mode == DATACD_INDEX) || (mode == BOOTABLECD_INDEX) ||
					 (mode == DVDVIDEO_INDEX));
}
...
bool ListView1::InitiateDrag(BPoint point, bool wasSelected)
{
	...
	/*
		if ((CDMode==DATACD_INDEX) || (CDMode==BOOTABLECD_INDEX)) {	// yes? then we are not in
	   audio mode
	...
	if ((CDMode == AUDIOCD_INDEX) || (CDMode == CDEXTRA_INDEX)) {
		audioRow = new AudioRow(((AudioRow*)row)->GetTrackNumber(), ((AudioRow*)row)->GetFilename(),
								((AudioRow*)row)->GetPregap(), ((AudioRow*)row)->GetBytes(),
	...
	if (GetRowRect(row, &rect)) {
		...
		if (!((FolderRow*)row)->IsFolder()) message->AddInt32("index", (int32)IndexOf(row));
...
void ListView1::MessageDropped(BMessage* msg, BPoint point)
{
	...
	case B_SIMPLE_DATA:
		...
		break;
	case B_MOUSE_MOVED:
		printf("B_MOUSE_MOVED\n");
		break;
	case LISTITEM_DROPPED:
		printf("LISTITEM_DROPPED\n");
		...
		if (msg->WasDropped()) {
			...
			if ((CDMode == DATACD_INDEX) || (CDMode == BOOTABLECD_INDEX) ||
				(CDMode == DVDVIDEO_INDEX)) { // yes? then we are not in audio mode
				draggedRow = folderRow;
			} else if ((CDMode == AUDIOCD_INDEX) || (CDMode == CDEXTRA_INDEX)) {
				draggedRow = audioRow;
			}
			void* row;
		...
		break;
	default:
		BColumnListView::MessageDropped(msg, point);
	}
}
...
void ListView1::AddAudioTrack(const char* name, size_t size, float length, int32** indeces,
							  const char* title)
{ // length in seconds
}
...
int32 ListView1::iCurrentSelection(int32 index)
{
	...
	if (CurrentSelection(index) == NULL)
		...
		i = IndexOf(CurrentSelection(index));
	return i;
}
