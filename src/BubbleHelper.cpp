...
BubbleHelper::BubbleHelper()
{
	...
	if (atomic_add(&runcount, 1) == 0) {
		...
		if (helperthread >= 0) resume_thread(helperthread);
		enabled = true;
	} else {
...
BubbleHelper::~BubbleHelper()
{
	...
	if (helplist) {
		...
		while (i >= 0) {
			...
			if (pair && pair->text) free(pair->text);
			delete pair;
			i--;
		}
		delete helplist;
...
char* BubbleHelper::GetHelp(BView* view)
{
	...
	while ((pair = (helppair*)helplist->ItemAt(i++)) != NULL) {
		if (pair->view == view) return pair->text;
	}
	return NULL;
}
...
void BubbleHelper::HideBubble()
{
	...
	if (!textwin->IsHidden()) textwin->Hide();
}
...
void BubbleHelper::ShowBubble(BPoint dest)
{
	...
	if (textwin->IsHidden()) textwin->Show();
}
...
void BubbleHelper::DisplayHelp(char* text, BPoint where)
{
	...
	for (int i = 0; i < numlines; i++)
		if ((linewidth = int(textview->LineWidth(i))) > width) width = linewidth;
	...
	if ((dest.y + height) > (screenframe.bottom - 3)) dest.y = dest.y - (16 + height + 8);
