...
ColorConfigView::ColorConfigView(BRect frame) : ConfigView(frame)
{
	testCTV = new ColorTestView(BRect(15, 100, 80, 110), "ccv_testview");
	AddChild(testCTV);

	mainCC = new BColorControl(BPoint(15, 5), B_CELLS_32x8, 6, "ccv_colorcontrol",
							   new BMessage(CTV_COLOR_UPDATE_MSG));
	AddChild(mainCC);
	// mainCC->ResizeTo(mainCC->Bounds().Width(), mainCC->Bounds().Height()+4);

	// list
	itemLV = new BListView(
		BRect(17, mainCC->Frame().bottom + 7, frame.Width() - 30, Bounds().bottom - 7),
		"ccv_selectionlist", B_SINGLE_SELECTION_LIST, B_FOLLOW_ALL);
	AddChild((itemSV = new BScrollView("ccv_scrollview", itemLV, B_FOLLOW_LEFT | B_FOLLOW_TOP, 0,
									   false, true)));
...
void ColorConfigView::AllAttached()
{
	...
	testCTV->ResizeTo(Bounds().right - mainCC->Frame().right - 30, mainCC->Bounds().Height());

	mainCC->SetTarget(this);

	//	itemSV->ResizeTo((mainCC->Frame().Width()-red->Frame().Width())-5,
	//Bounds().Height()-(mainCC->Frame().bottom+22));
...
void ColorConfigView::MessageReceived(BMessage* msg)
{
	...
	case CTV_COLOR_UPDATE_MSG: {
		...
		if (itemLV->CurrentSelection() >= 0) {
			((ColorListItem*)itemLV->ItemAt(itemLV->CurrentSelection()))
				->SetColor(mainCC->ValueAsColor());
	...
	case LIST_SELECTION_MSG: {
		if (itemLV->CurrentSelection() >= 0) {
			...
			mainCC->SetValue(
				((ColorListItem*)itemLV->ItemAt(itemLV->CurrentSelection()))->GetColor());
			itemLV->InvalidateItem(itemLV->CurrentSelection());
			testCTV->SetTestColor(mainCC->ValueAsColor());
		} else
			mainCC->SetEnabled(false);
