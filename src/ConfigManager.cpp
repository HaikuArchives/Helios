...
void ConfigManager::SetInt64(const char *name, int64 value, int32 index) {
	if (configMsg->ReplaceInt64(name, index, value)!=B_OK)
		configMsg->AddInt64(name, value);
}

int64 *ConfigManager::GetInt64(const char *name, int32 index) {
	int64 value;
	if (configMsg->FindInt64(name, index, &value)!=B_OK)
		return NULL;
...
void ConfigManager::SetFloat(const char *name, float value, int32 index) {
	if (configMsg->ReplaceFloat(name, index, value)!=B_OK)
		configMsg->AddFloat(name, value);
}

float *ConfigManager::GetFloat(const char *name, int32 index) {
	float value;
	if (configMsg->FindFloat(name, index, &value)!=B_OK)
		return NULL;
