...
ConfigWindow::ConfigWindow(const char* title)
	: BWindow(BRect(0, 0, 600, 300), title, B_TITLED_WINDOW,
			  B_NOT_ZOOMABLE | B_NOT_RESIZABLE | B_ASYNCHRONOUS_CONTROLS)
{
	...
	selectionLV =
		new BOutlineListView(BRect(3, 3, 160, Bounds().bottom), "selectionLV",
							 B_SINGLE_SELECTION_LIST, B_FOLLOW_LEFT | B_FOLLOW_TOP_BOTTOM);

	scrollSV = new BScrollView("scrollSV", selectionLV, B_FOLLOW_LEFT | B_FOLLOW_TOP_BOTTOM, 0,
...
void ConfigWindow::RefreshConfigView()
{
	if (selectionLV->FullListCurrentSelection() >= 0) {
		if (boundingBOX->CountChildren() > 0) boundingBOX->RemoveChild(boundingBOX->ChildAt(0));
		if (selectionLV->FullListCurrentSelection() >= 0) {
			boundingBOX->AddChild(
				(BView*)(viewsL->ItemAt(selectionLV->FullListCurrentSelection())));
