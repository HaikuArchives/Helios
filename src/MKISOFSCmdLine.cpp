...
//				argv[(*argc)++] = strdup("-l");
			...
			if ((view1->IsMultisession()) && (!printsize)) {
				...
				// session of the disc
				ErrorBox *alert=new ErrorBox(E_ORANGE_COLOR, "", 	_T("Is this session going to be the first, the last or another session of a multisession disc?"), // "MULTISESSION_ALERT"
				...
				if (selected!=2)
					if (app->GetMultisessionSectors(start_sec, end_sec)!=B_OK) {
						...
						app->errstring = _T("Could not read inserted multisession disc!"); // "MULTISESSION_FAILS_ALERT"
						return B_ERROR;
					}
				BString		text="";
