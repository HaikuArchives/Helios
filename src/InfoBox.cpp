...
InfoBox::InfoBox()
	: BBox(BRect(317, 90, 589, 169), "box", B_FOLLOW_TOP | B_FOLLOW_LEFT, B_WILL_DRAW,
		   B_FANCY_BORDER)
{
	...
	stringview3 =
		new BStringView(BRect(8, 9, 134, 21), "StringView", _T("Total CD size:"), // "TTOTALCDSIZE"
						B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW);
	stringview3->SetFontSize(10);
	//	AddChild(stringview3);

	stringview9 = new BStringView(BRect(8, 23, 134, 35), "StringView",
								  _T("Total CD duration:"), // "TCDLENGTH"
								  B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW);
	stringview9->SetFontSize(10);
	//	AddChild(stringview9);

	stringview4 = new BStringView(BRect(8, 37, 134, 49), "StringView",
								  _T("Estimated time:"), // "TESTIMATEDTIME"
								  B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW);
	stringview4->SetFontSize(10);
	//	AddChild(stringview4);

	filesSV =
		new BStringView(BRect(7, 5, 134, 20), "StringView", _T("Number of files:")); // "TFILECOUNT"
	filesSV->SetFontSize(10);
	AddChild(filesSV);

	foldersSV = new BStringView(BRect(7, 21, 134, 36), "StringView",
								_T("Number of folders:")); // "TFOLDERCOUNT"
	foldersSV->SetFontSize(10);
	AddChild(foldersSV);

	stringview5 = new BStringView(BRect(135, 9, 264, 21), "StringView", "0.00MB",
	...
	// UPDATE BUTTON
	//	button3=new BButton(BRect(204,48,264,74),"Button3",TUPDATE,new
	//BMessage(UPDATEINFO_MSG),B_FOLLOW_RIGHT | B_FOLLOW_TOP,B_WILL_DRAW);
...
void InfoBox::AttachedToWindow()
{
	if (warningBM == NULL)
		warningBM =
			new BBitmap(BRect(0, 0, kDataWidth - 1, kDataHeight - 1), B_RGBA32, false, true);
...
void InfoBox::UpdateSizes()
{
	...
	default: {
		levelSB->SetMaxValue(
			(((Application1*)be_app)->heliosCV->IsOverburning() ? 405000.0F : 360000.0F));
		break;
	}
	}
	...
	//	stringview10->SetHighColor(0,0,0,255);
		...
		float
	r=(size_t)(((size_t)cd_size-((size_t)((float)cd_size/1048576.0F))*1048576.0F)/10485.76F);
		if (r<10) s="0"; else s="";
		...
		if (((Application1 *)be_app)->window1->view1->CDType()==DVDVIDEO_INDEX)
			...
			levelSB->SetMaxValue( (((Application1
	*)be_app)->heliosCV->IsOverburning()?91.0F:80.0F)*60.0F);
	...
	//		stringview10->SetHighColor(200,0,0,255);
		...
		r=(2352*(size_t)size2/(2048*176400*(size_t)(((Application1
	*)be_app)->heliosCV->GetWriterSpeed())));
		str="";
...
void InfoBox::SpawnUpdateThread()
{
	if (!ut_running) {
		...
		d.seconds = 0;
		update_Thread =
			spawn_thread(UpdateThread, UPDATETHREADNAME, UPDATETHREAD_PRIORITY, (void*)(&d));
	}
}
...
int32 InfoBox::UpdateThread(void* data)
{
	...
	while (true) {
		...
		for (i = 0; i < dt->seconds * 4; i++) {
			snooze(250000);
			if (dt->interruptFlag) i = dt->seconds * 4 + 1;
		}
