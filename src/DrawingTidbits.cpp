...
{
	rgb_color result = {ShiftComponent(color.red, percent), ShiftComponent(color.green, percent),
						ShiftComponent(color.blue, percent), 0};
...
void ReplaceColor(BBitmap* bitmap, rgb_color from, rgb_color to)
{
	...
	for (int32 index = 0; index < bitsLength; index++)
		if (bits[index] == fromIndex) bits[index] = toIndex;
}
...
void ReplaceTransparentColor(BBitmap* bitmap, rgb_color with)
{
	...
	int32 bitsLength = bitmap->BitsLength();
	for (int32 index = 0; index < bitsLength; index++)
		if (bits[index] == B_TRANSPARENT_8_BIT) bits[index] = withIndex;
}
