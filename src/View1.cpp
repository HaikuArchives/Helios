...
View1::View1(BRect rect) : BView(rect, "View1", B_FOLLOW_ALL_SIDES, B_WILL_DRAW)
{
	BString tmpstring;
	...
	textcontrol2 = new BTextControl(BRect(415, 55, 435 + StringWidth(_T("Pause")) + 8 + 30, 75),
									"tc", _T("Pause"), NULL, new BMessage(PAUSECHANGE_MSG),
	...
	*/
	...
	// menufield2 = new BMenuField(BRect(415, 25, 589, 45), "menufield", TFILESYSTEM, popupmenu2,
	// new BMessage(FILESYSTEM_MSG), B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW);
	//	menufield2->SetDivider(StringWidth(TFILESYSTEM)+8);
	//	menuitem3->SetEnabled(false);

	// submenu-items for File-menu
	newprojectMI =
		new BMenuItem(_T("New project"), new BMessage(NEWPROJECT_MIMSG), 'N'); // "TMI_NEWPROJECT"
	...
	saveprojectMI = new BMenuItem(_T("Save project"), new BMessage(SAVEPROJECT_MIMSG),
								  'S'); // "TMI_SAVEPROJECT"
	...
	// standardprojectMI=new BMenuItem(_T("TMI_STANDARDPROJECT"), new
	// BMessage(STANDARDPROJECT_MIMSG));
	...
	export_M3U =
		new BMenuItem(_T("CL-Amp playlist"), new BMessage(EXPORT_M3U_MIMSG)); // "TMI_EXPORT_M3U"
	export_M3U->SetTarget(be_app_messenger);
	exportSM->AddItem(export_M3U);
	// EXPORT AS APML PLAYLIST
	export_APML =
		new BMenuItem(_T("APlayer playlist"), new BMessage(EXPORT_APML_MIMSG)); // "TMI_EXPORT_APML"
	export_APML->SetTarget(be_app_messenger);
	exportSM->AddItem(export_APML);
	// EXPORT AS PLAIN FILELIST
	export_FileList = new BMenuItem(_T("Plain filelist"),
									new BMessage(EXPORT_FILELIST_MIMSG)); // "TMI_EXPORT_FILELIST"
	export_FileList->SetTarget(be_app_messenger);
	exportSM->AddItem(export_FileList);
	// EXPORT AS TRACKER TEMPLATE
	// export_TrackerTemplate=new BMenuItem(_T("MI:Tracker Template"), new
	// BMessage(EXPORT_TRACKERTEMPLATE_MIMSG));
	// export_TrackerTemplate->SetTarget(be_app_messenger);
	// exportSM->AddItem(export_TrackerTemplate);
	removeprojectMI = new BMenuItem(_T("Remove current project"),
									new BMessage(REMOVEPROJECT_MIMSG)); // "TMI_REMOVEPROJECT"
	...
	readaudiocdMI =
		new BMenuItem(_T("Read Audio CD"), new BMessage(READAUDIOCD_MIMSG)); // "MI:Read Audio CD"
	readaudiocdMI->SetTarget(be_app_messenger);
	cdimgSM->AddItem(readaudiocdMI);

	// submenu-item for cd-copy
	copyaudiocdMI =
		new BMenuItem(_T("Copy Audio CD"), new BMessage(COPYAUDIOCD_MIMSG)); // "TMI_COPYAUDIOCD"
	...
	copydatacdMI =
		new BMenuItem(_T("Copy Data CD"), new BMessage(COPYDATACD_MIMSG)); // "TMI_COPYDATACD"
	copydatacdMI->SetTarget(be_app_messenger);
	copycdM->AddItem(copydatacdMI);

	// submenu-tems for cd-rw
	fastblankMI =
		new BMenuItem(_T("Entire CD (fast)"), new BMessage(FASTBLANK_MIMSG)); // "TMI_FASTBLANK"
	fastblankMI->SetTarget(be_app_messenger);
	rwM->AddItem(fastblankMI);
	completeblankMI = new BMenuItem(_T("Entire CD (complete)"),
									new BMessage(COMPLETEBLANK_MIMSG)); // "TMI_COMPLETEBLANK"
	completeblankMI->SetTarget(be_app_messenger);
	rwM->AddItem(completeblankMI);
	unclosesessionMI = new BMenuItem(_T("Open last session"),
									 new BMessage(UNCLOSESESSION_MIMSG)); // "TMI_UNCLOSESESSION"
	unclosesessionMI->SetTarget(be_app_messenger);
	rwM->AddItem(unclosesessionMI);
	blanksessionMI = new BMenuItem(_T("Erase last session"),
								   new BMessage(BLANKSESSION_MIMSG)); // "TMI_BLANKSESSION"
	...
	// AddChild(new
	// ScrollViewCorner(scrollview1->Frame().right-B_V_SCROLL_BAR_WIDTH,scrollview1->Frame().bottom-B_H_SCROLL_BAR_HEIGHT));
	AddChild(listview1);
	...
	statusBAR = new StatusBar(BRect(0, 0, 1, 1), "v1_statusBAR", B_FOLLOW_LEFT | B_FOLLOW_BOTTOM,
							  B_WILL_DRAW);
	AddChild(statusBAR);
}
...
void View1::AllAttached(void)
{
	...
	((BButton*)mainBAR->ChildViewAt(
		 mainBAR->AddButton("New Project", "bar_newproject", new BMessage(NEWPROJECT_MIMSG))))
		->SetTarget(be_app_messenger);
	((BButton*)mainBAR->ChildViewAt(
		 mainBAR->AddButton("Open Project", "bar_openproject", new BMessage(OPENPROJECT_MIMSG))))
		->SetTarget(be_app_messenger);
	((BButton*)mainBAR->ChildViewAt(
		 mainBAR->AddButton("Save Project", "bar_saveproject", new BMessage(SAVEPROJECT_MIMSG))))
		->SetTarget(be_app_messenger);
	//	((BButton *)mainBAR->ChildViewAt(mainBAR->AddButton("Save Project As", "", new
	//BMessage(SAVEPROJECTAS_MIMSG))))->SetTarget(be_app_messenger);
	mainBAR->AddChildView(new IconBarSeparator(BRect(0, 0, 1, 22), "separator"));
	((BButton*)mainBAR->ChildViewAt(mainBAR->AddButton(
		 5, "bar_preferences", new BMessage(PREFERENCES_MIMSG))))->SetTarget(be_app_messenger);
	mainBAR->AddChildView(new IconBarSeparator(BRect(0, 0, 1, 22), "separator"));
	((BButton*)mainBAR->ChildViewAt(mainBAR->AddButtonMSGG(
		 "Burn Now", "bar_burnnow", new BMessage(BURN_MSG))))->SetTarget(be_app_messenger);
	mainBAR->AddChildView(new IconBarSeparator(BRect(0, 0, 1, 22), "separator"));
	AddChild(mainBAR);
	mainBAR->MoveTo(0, menu_height + 1);
	mainBAR->ResizeTo(list_left, 30);

	iconBAR = new IconBar(BRect(0, 0, listview1->Bounds().Width(), 30), "v1_iconBAR",
						  B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP);
	//	((BButton *)iconBAR->ChildViewAt(iconBAR->AddButton("New Folder", "bar_newfolder", new
	//BMessage('mm12'))))->SetTarget(be_app_messenger);
	((BButton*)iconBAR->ChildViewAt(
		 iconBAR->AddButton("New Folder", "bar_newfolder", new BMessage(NEW_FOLDER_MSG))))
		->SetTarget(be_app_messenger);
	((BButton*)iconBAR->ChildViewAt(iconBAR->AddButton(
		 1, "bar_deleteitem", new BMessage(DELETE_FILE_MSG))))->SetTarget(be_app_messenger);
	iconBAR->AddChildView(new IconBarSeparator(BRect(0, 0, 1, 22), "separator"));
	iconBAR->AddChildView(
		(IBpathDisplayTC = new BTextControl(BRect(0, 0, 222, 20), "bar_path", "", "[no path]", NULL,
											B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP_BOTTOM)));
	((BButton*)iconBAR->ChildViewAt(iconBAR->AddButton(
		 2, "bar_parent", new BMessage(GOTOPARENT_MSG), B_FOLLOW_RIGHT | B_FOLLOW_TOP)))
		->SetTarget(be_app_messenger);
	iconBAR->AddChildView(
		new IconBarSeparator(BRect(0, 0, 1, 22), "separator", B_FOLLOW_RIGHT | B_FOLLOW_TOP));
	((BButton*)iconBAR->ChildViewAt(iconBAR->AddButton(
		 4, "bar_update", new BMessage(UPDATEINFO_MSG), B_FOLLOW_RIGHT | B_FOLLOW_TOP)))
		->SetTarget(be_app_messenger);
	AddChild(iconBAR);
...
void View1::Draw(BRect updateRect)
{
	...
	StrokeLine(BPoint(0, iconBAR->Frame().bottom + 1),
			   BPoint(list_left - 1, iconBAR->Frame().bottom + 1), B_SOLID_HIGH);
...
void View1::SetFilesystemBootCD(int32 index)
{
	if (index < bootablepanel->filesystemPUM->CountItems())
		...
		bootablepanel->filesystemPUM->ItemAt(bootablepanel->filesystemPUM->CountItems() - 1)
			->SetMarked(true);
}
