...
static unsigned short updcrc(p_crc, cp, cnt) unsigned int p_crc;
...
static unsigned short calcCRC(buf, bsize) unsigned char* buf;
unsigned bsize;
{
...
static int flip_error_corr(b, crc) unsigned char* b;
int crc;
{
	if (crc != 0) {
		int i;
...
static int cdtext_crc_ok(c) cdtextpackdata* c;
{
	int crc;
	int retval;
...
static void dump_binary(c) cdtextpackdata* c;
{
	fprintf(stderr, ": header fields %02x %02x %02x  ", c->headerfield[1], c->headerfield[2],
			c->headerfield[3]);
	fprintf(stderr, "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
			c->textdatafield[0], c->textdatafield[1], c->textdatafield[2], c->textdatafield[3],
			c->textdatafield[4], c->textdatafield[5], c->textdatafield[6], c->textdatafield[7],
			c->textdatafield[8], c->textdatafield[9], c->textdatafield[10], c->textdatafield[11]);
}
#endif

static int process_header __PR((cdtextpackdata * c, int dbcc, unsigned char* line));

static int process_header(c, dbcc, line) cdtextpackdata* c;
int dbcc;
unsigned char* line;
{
	...
	case 0x80: /* Title of album or track */
		...
		if (c->headerfield[1] > 0 && c->headerfield[1] < 100 &&
			...
			if (len > 0) global.tracktitle[c->headerfield[1]] = malloc(len + 1);
		...
		} else if (c->headerfield[1] == 0 && global.disctitle == NULL) {
			unsigned len;

			len = strlen((char*)line);

			if (len > 0) global.disctitle = malloc(len + 1);
	...
	case 0x81: /* Name(s) of the performer(s) */
		...
		if (c->headerfield[1] > 0 && c->headerfield[1] < 100 &&
			...
			if (len > 0) global.trackcreator[c->headerfield[1]] = malloc(len + 1);
		...
		} else if (c->headerfield[1] == 0 && global.creator == NULL) {
			unsigned len;

			len = strlen((char*)line);

			if (len > 0) global.creator = malloc(len + 1);
	...
	case 0x8e: /* UPC/EAN code or ISRC code */
		...
		} else if (c->headerfield[1] == 0 && line[0] != '\0') {
			Set_MCN(line);
		}
	...
	case 0x8f: /* Size information of the block */
		...
		case 1:
			...
			fprintf(stderr, "last sequence numbers for blocks 1-8: %d %d %d %d ",
					c->textdatafield[8], c->textdatafield[9], c->textdatafield[10],
					c->textdatafield[11]);
			break;
		case 2:
			fprintf(stderr, "%d %d %d %d\n", c->textdatafield[0], c->textdatafield[1],
					c->textdatafield[2], c->textdatafield[3]);
			fprintf(stderr, "Language codes for blocks 1-8: %d %d %d %d %d %d %d %d\n",
					c->textdatafield[4], c->textdatafield[5], c->textdatafield[6],
					c->textdatafield[7], c->textdatafield[8], c->textdatafield[9],
					c->textdatafield[10], c->textdatafield[11]);
			break;
		}
