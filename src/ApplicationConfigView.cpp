...
ApplicationConfigView::ApplicationConfigView(BRect frame) : ConfigView(frame)
{
	...
	// SLIDERS
	updatefreqSL = new BSlider(BRect(15, 25, frame.Width() - 15, 55), "acv_updatefreqSL", NULL,
							   new BMessage(UPDATEFREQSLIDER_CHANGED), 1, 20);
	...
	// CHECKBOXES
	timeoutCB =
		new BCheckBox(BRect(15, 60, 230, 75), "acv_timeoutCB", _T("Timeouts"), // "TENABLETIMEOUT"
					  new BMessage(TIMEOUTCHECKBOX_CLICKED));
	timeoutCB->ResizeToPreferred();
	AddChild(timeoutCB);

	// SLIDERS
	timeoutSL = new BSlider(BRect(15, 80, frame.Width() - 15, 105), "acv_timeoutSL", NULL,
							new BMessage(TIMEOUTSLIDER_CHANGED), 2, 90);
