...
bool
FileCopy(const char *source, const char *destination, size_t srcfilesize)
{
	bool	retval = true;
	...
	if ((src->InitCheck() == B_OK) && (dst->InitCheck() == B_OK)) {
		...
		while ((dst->Write(buffer, numread) == numread) && (buffersize >= sectorsize)) {
			...
			if (progressed > 64*1024*8) {
					sprintf(info,"%0.1f MB/s",((float)progressed)/((float)watch->ElapsedTime()*1.048576));
...
bool
CreateFolder(const char *folder_name)
{
...
bool
CreateEntry(char *pathname, char *entryname)
{
	...
	if (!haveEncounteredProjectFolder) {
		...
		if (entry.IsDirectory()) {
			...
			if (app->filecounter % 20==0) {
				if (app->window1->Lock()) {
					BString tmpstring;
					tmpstring << _T("Adding files") << B_UTF8_ELLIPSIS << " (" << app->filecounter << ")"; // "L:Adding files..."
			...
			while (dir.GetNextEntry(&de, false) == B_OK) {
				...
				if (de.IsDirectory()) {
					...
					app->cdir.Remove(app->cdir.FindLast("/"), app->cdir.Length() - app->cdir.FindLast("/"));
					//printf("PARENTDIR %s\n",cdir.String());
				}
	...
	if (depthCount == 0) {
		if (haveEncounteredProjectFolder) {
			...
			ErrorBox *eb=new ErrorBox(E_RED_COLOR,	_T("Error"), // "TERROR"
				_T("No files from within the Project folder can be added - any such files are ignored."), // "TFOUNDPROJECTFILE"
				_T("Ok")); // "TOK"
			eb->Go();
		}
	}
	...
	if (count==1) {
		if (msg->FindRef("refs", &ref)==B_OK) {
			...
			if (strcmp(type, HELIOSPROJECTMIMETYPE)==0) {
				ErrorBox	*eb=new ErrorBox(E_BLUE_COLOR, 	_T("Project file dropped"), // "TPROJECTFILEDROPPED"
															_T("You just dropped a Helios project file. Do you want to add this file to the current project or load this project with Helios?"), // "TPROJECTFILEDROPPED_TEXT"
															_T("Load"), 				// "TLOAD"
															_T("Add"));
				switch(eb->Go()) {
					...
					if (entry->GetSize(&filesize)==B_OK) {
						...
						if ((cdtype==AUDIOCD_INDEX) || (cdtype==CDEXTRA_INDEX)) {
							...
							if (size>0) {
								if (app->window1->Lock()) {
									AudioRow	*ar;
									app->window1->view1->listview1->AddRow((ar=new AudioRow(app->window1->view1->listview1->iCountItems()+1, path.Leaf(), 0, size, size/176400)));
...
removeFiles(void *data)
{
	...
	if (app->window1->Lock()) {
		while ((row=app->window1->view1->listview1->CurrentSelection())) {
			if ((cdtype==AUDIOCD_INDEX) || (cdtype==CDEXTRA_INDEX)) filename=((AudioRow *)(row))->GetFilename();
			...
			if (filename.Compare("..")!=0) {	     		
				...
				if ((cdtype==AUDIOCD_INDEX) || (cdtype==CDEXTRA_INDEX)) {
					for (int j=0; j<app->window1->view1->listview1->iCountItems();j++) {
						if (str.Compare(((AudioRow *)(app->window1->view1->listview1->RowAt(j)))->GetFilename())==0) {
				...
				((BButton *)app->window1->view1->iconBAR->ChildViewAt("bar_deleteitem"))->SetEnabled(app->window1->view1->listview1->CurrentSelection()!=NULL);
			}
		}
		app->window1->Unlock();
	}
	...
	for (int32 k=list.CountItems()-1; k>=0; k--) {
		...
		if (fileEntry->SetTo(str.String(),false) == B_OK) {
			...
			if (cdtype==BOOTABLECD_INDEX) {
				...
				if (cdpathtoimage.Compare(app->window1->view1->bootablepanel->GetImageName())==0) {
					ErrorBox	*eb=new ErrorBox(E_ORANGE_COLOR, 	_T("Delete floppy image?"), // "Error:Delete Floppy Image?"
																	_T("If you click 'Yes' the path will be removed from your project's settings. Click 'No' to keep the file."), // "Error:Delete Floppy Image?Error text"
																	_T("Yes"),
																	_T("No"));
					if (eb->Go() == 1) {
						fileEntry->Unset();
						...
						if (app->window1->Lock()) {
							app->window1->view1->listview1->Deselect(app->window1->view1->listview1->CurrentSelection());
							app->window1->Unlock();
						}
						continue;
					} else {
			...
			if (app->filecounter % 20==0) {
				if (app->window1->Lock()) {
					BString tmpstring;
					tmpstring << _T("Removing files") << B_UTF8_ELLIPSIS << " (" << app->filecounter << ")"; // "L:Removing files..."
					app->window1->view1->statusBAR->SetText( tmpstring.String() );
					app->window1->Unlock();
				}
			}
...
void
GetDirectorySize(BDirectory& dir, int32& numFiles, int32& numDirs, off_t& totalSize)
{
...
int
MountBFSImage(const char *filename, const char *mountpoint)
{
	//+MSH: TODO - replace dependency on be_app.
	Application1* app = dynamic_cast<Application1*>(be_app);
...
//		sprintf(info,"%s",_T("TWAITING"));
		...
		app->SetPercentage((float)timeout*100/((float)app->applicationCV->GetTimeoutLength()/5));
	};
...
int
UnmountBFSImage(const char *mountpoint)
{
	//+MSH: TODO - replace dependency on be_app.
	Application1* app = dynamic_cast<Application1*>(be_app);
...
status_t
BurnImage(const BString& filename)
{
	//+MSH: TODO - replace dependency on be_app.
	Application1* app = dynamic_cast<Application1*>(be_app);
	...
	if (app->fStatusWindow->Lock()) {
		if (app->heliosCV->IsSimulation()) {
			app->fStatusWindow->SetTitle(_T("Helios - SIMULATION")); // "TSTATUSWINDOWTITLESIMULATION"
		} else {
	...
	if (app->window1->Lock()) {
		...
		if (file->InitCheck() == B_OK) {
			...
			if (!app->CheckCDSize()) {
				...
				app->errstring=_T("The project data size is greater than the maximum size of a CD. Remove some files or select 'Overburning' in the preferences panel."); // "TOVERSIZED"
				delete file;
				return B_ERROR;
			}
		}
	...
	CDRECORD_CommandLine(args, &arguments, app->window1->view1->IsMultisession(), false /*isStreaming*/);
...
const char *
CheckCommandlineTool(const char *fullpath, const char *name)
{
	BString string, path;
	...
	if (cdrecord.SetTo(fullpath, B_READ_ONLY) != B_OK) {
		...
		// "TXNOTFOUND"
		BString notFoundStr = _T("NAME not found! It has to be installed to use Helios. Press 'Search' if you want Helios to search it for or press 'Cancel' to exit.");
...
void
OutPutRAW(const char *filename, int fd)
{
	char *buffer(NULL);
