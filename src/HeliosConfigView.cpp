...
HeliosConfigView::HeliosConfigView(BRect frame) : ConfigView(frame){
	// CHECKBOXES
	simulationCB=new BCheckBox(BRect(15,5,230,20), "hcv_simulationCB", _T("Simulation"), // "TSIMULATION"
					NULL);
	AddChild(simulationCB);
	fixatingCB=new BCheckBox(BRect(15,25,230,40), "hcv_fixatingCB", _T("Fixate"), // "TFIXATE"
					NULL);
	AddChild(fixatingCB);
	burnproofCB=new BCheckBox(BRect(15,45,230,60), "hcv_burnproofCB", _T("BurnProof"), // "TBURNPROOF"
					NULL);
	AddChild(burnproofCB);

	forcedCB=new BCheckBox(BRect(235, 5, (frame.right - 15), 20), "hcv_forcedCB", _T("Force RW-erasing"), // "TFORCEOPTION"
					NULL);
	AddChild(forcedCB);
	DAOCB=new BCheckBox(BRect(235,25,(frame.right - 15),40), "hcv_DAOCB", _T("Disc-at-Once"), // "TDAOMODE"
					new BMessage(DAO_MSG));
	AddChild(DAOCB);
	overburningCB=new BCheckBox(BRect(235,45,(frame.right - 15),60), "hcv_overburningCB", _T("allow CDs > 80 min"), // "TALLOWOVERBURNING"
					NULL);
	AddChild(overburningCB);

//	generalFlagsTC=new BTextControl(BRect(15,115,(frame.right - 15),130), "hcv_generalTC", _T("General Flags"),
	generalFlagsTC=new BTextControl(BRect(15,65,(frame.right - 15),80), "hcv_generalTC", _T("General Flags"),
			"",NULL);
...
//	speedSL=new BSlider(BRect(15, 164, 325, 198), "hcv_speedSL", NULL, new BMessage(SPEEDSLIDER_CHANGED), 0, 24);
	speedSL=new BSlider(BRect(15, 164, viewRect.right - 15, 198), "hcv_speedSL", NULL, new BMessage(SPEEDSLIDER_CHANGED), 0, 24);
	...
	AddChild(speedSL);
	fifoSL=new BSlider(BRect(15, 216, viewRect.right - 15, 250), "hcv_fifoSL", NULL, new BMessage(FIFOSLIDER_CHANGED), 1, 32);
...
BString
HeliosConfigView::GetGeneralFlags() {
	return generalFlagsTC->Text();
}

void
HeliosConfigView::SetGeneralFlags(BString general) {
		...
		case DAO_MSG: {
			...
			if (app->window1->Lock()) {
				if (app->window1->view1->datapanel->IsMultisession()) {
					...
					if (app->window1->Lock()) {
						app->window1->view1->statusBAR->SetText(_T("Multisession disabled."), // "L:Multisession disabled."
								4000000);
						app->window1->Unlock();
					}
				}
...
void	HeliosConfigView::GetSettings(BMessage *archive) {
	// CHECKBOXES
	archive->AddBool("HeliosConfigView::hcv_simulationCB", ((BCheckBox *)FindView("hcv_simulationCB"))->Value()==B_CONTROL_ON);
	archive->AddBool("HeliosConfigView::hcv_fixatingCB", ((BCheckBox *)FindView("hcv_fixatingCB"))->Value()==B_CONTROL_ON);
	archive->AddBool("HeliosConfigView::hcv_burnproofCB", ((BCheckBox *)FindView("hcv_burnproofCB"))->Value()==B_CONTROL_ON);
	archive->AddBool("HeliosConfigView::hcv_forcedCB", ((BCheckBox *)FindView("hcv_forcedCB"))->Value()==B_CONTROL_ON);
	archive->AddBool("HeliosConfigView::hcv_DAOCB", ((BCheckBox *)FindView("hcv_DAOCB"))->Value()==B_CONTROL_ON);
	archive->AddBool("HeliosConfigView::hcv_overburningCB", ((BCheckBox *)FindView("hcv_overburningCB"))->Value()==B_CONTROL_ON);

	// SLIDERS
	archive->AddInt32("HeliosConfigView::hcv_speedSL", ((BSlider *)FindView("hcv_speedSL"))->Value());
	archive->AddInt32("HeliosConfigView::hcv_fifoSL", ((BSlider *)FindView("hcv_fifoSL"))->Value());
	archive->AddString("HeliosConfigView::hcv_generalTC", ((BTextControl *)FindView("hcv_generalTC"))->Text());
}
...
void	HeliosConfigView::SetSettings(BMessage *archive) {
	...
	if (archive->FindBool("HeliosConfigView::hcv_simulationCB", &boolvar)==B_OK)
		((BCheckBox *)FindView("hcv_simulationCB"))->SetValue(boolvar?B_CONTROL_ON:B_CONTROL_OFF);
	else
	...
	if (archive->FindBool("HeliosConfigView::hcv_overburningCB", &boolvar)==B_OK)
		((BCheckBox *)FindView("hcv_overburningCB"))->SetValue(boolvar?B_CONTROL_ON:B_CONTROL_OFF);
	else
