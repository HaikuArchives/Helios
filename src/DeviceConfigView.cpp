...
DeviceConfigView::DeviceConfigView(BRect frame) : ConfigView(frame){
	...
	if( width < StringWidth( _T( "CD Reader" ) ) )
		width = StringWidth( _T( "CD Reader" ) );
	width += 5;

	// CD Writer MenuField
	writerPUM=new BPopUpMenu("                            ");
	writerMF=new BMenuField(BRect(15,5,290,25),"dcv_cdwriterMF",
					_T("CD Writer"), // "TCDRDRIVE"
					writerPUM);
	...
	defaultRB=new BRadioButton(BRect(20, 20, scanmethodBOX->Bounds().right-15, 35), "dcv_defaultRB",
					_T("Default method"), // "RB:Default Method"
					new BMessage(METHODCHANGED));
	scanmethodBOX->AddChild(defaultRB);
	secondRB=new BRadioButton(BRect(20, 40, scanmethodBOX->Bounds().right-15, 55), "dcv_secondRB",
					_T("Second method"), // "RB:Second Method"
					new BMessage(METHODCHANGED));
	scanmethodBOX->AddChild(secondRB);
	emergencyRB=new BRadioButton(BRect(20, 60, scanmethodBOX->Bounds().right-15, 75), "dcv_emergencyRB",
...
void	DeviceConfigView::RescanDevices() {
	...
	for (int32 i=readerPUM->CountItems()-1; i>=0; i--)
		delete readerPUM->RemoveItem(i);
	for (int32 i=writerPUM->CountItems()-1; i>=0; i--)
		delete writerPUM->RemoveItem(i);
...
void	DeviceConfigView::SetWriterNumber(int no) {
	if (writerPUM->ItemAt(no)!=NULL)
		(writerPUM->ItemAt(no))->SetMarked(true);
}
...
void	DeviceConfigView::SetReaderNumber(int no) {
	if (readerPUM->ItemAt(no)!=NULL)
		(readerPUM->ItemAt(no))->SetMarked(true);
}
...
status_t get_device_path_target_lun
	(
	const char *	dev,
	int *		 	path,
	int *		 	target,
...
void DeviceConfigView::lookup_devices(const char *path, const char *appendthis) {
	...
	while (dir.GetNextEntry(&entry) >= 0)
		...
		if (entry.IsDirectory())
			lookup_devices(name.Path(), appendthis);

		if ( strcmp(name.Leaf(), "raw") != 0 )
			continue;
