...
CDExtraPanel::CDExtraPanel(BRect frame)
	: BView(frame, "CDExtraPanel", B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW)
{
	cdtextCB = new BCheckBox(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
							 "xcdp_writeCDText", _T("Write CD text"), // "TWRITECDTEXT"
							 new BMessage(CDTEXTBOX_MSG));
	copyprotectionCB =
		new BCheckBox(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
					  "xcdp_copyprotection", _T("Add SCMS-Copyright"), // "TCOPYPROTECTION"
					  NULL);
	CDTextAlbum =
		new BTextControl(BRect(frame.left + 4, frame.top, frame.right - 4, frame.top + 16),
						 "xcdp_cdtextalbum", _T("CD text album"), // "TCDTEXTALBUMLABEL"
						 "", NULL);
	// AddChild(cdtextCB);
	...
	imagenameTC =
		new BTextControl(BRect(0, 0, 200, 21), "xcdp_imagenameTC", _T("Data path"), // "TDATAIMAGE"
						 "", NULL);
...
void CDExtraPanel::MessageReceived(BMessage* msg)
{
	...
	case B_SIMPLE_DATA: {
		...
		if (msg->FindRef("refs", 0, &ref) == B_OK) {
			if (entry->SetTo(&ref, true) == B_OK) entry->GetPath(path);
	...
	//					//str.RemoveAll("//");
	//					//if (str.Length()==0) str="/";
	//					str2=((FolderRow *)(((Application1
	//*)be_app)->window1->view1->listview1->RowAt(i)))->GetFilename();
	//					str.Append(str2);
...
{
	if (flag)
		cdtextCB->SetValue(B_CONTROL_ON);
	else
...
{
	if (flag)
		copyprotectionCB->SetValue(B_CONTROL_ON);
	else
