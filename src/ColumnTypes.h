...
{
public:
	BStringColumn(const char* title, float width, float maxWidth, float minWidth, uint32 truncate,
				  alignment align = B_ALIGN_LEFT);
...
{
public:
	BBitmapStringColumn(const char* title, float width, float minWidth, float maxWidth,
...
{
public:
	BBitmapStringField(BBitmap* bitmap, const char* string);
...
{
public:
	BViewColumn(const char* title, float width, float minWidth, float maxWidth,
				alignment align = B_ALIGN_LEFT);
	virtual void DrawField(BField* field, BRect rect, BView* parent);
};
